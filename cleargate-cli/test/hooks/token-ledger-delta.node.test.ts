...
describe('Scenario 1: single-session multi-fire', () => {
  test('3 fires produce per-turn deltas; sum(delta) == last session_total', () => {
    ...
    assert.deepStrictEqual(row1['delta'], { input: 300, output: 150 });
    assert.deepStrictEqual(row1['session_total'], { input: 300, output: 150 });
    ...
    assert.deepStrictEqual(row2['delta'], { input: 200, output: 100 });
    assert.deepStrictEqual(row2['session_total'], { input: 500, output: 250 });
    ...
    // delta = 700 - 500 = 200
    assert.deepStrictEqual(row3['delta'], { input: 200, output: 100 });
    assert.deepStrictEqual(row3['session_total'], { input: 700, output: 350 });
    ...
    assert.strictEqual(totalDeltaInput, lastSessionTotalInput); // 300 + 200 + 200 = 700
  });
});
...
describe('Scenario 2: multi-session sprint', () => {
  test('4 rows across 2 sessions; sum(delta.input) == sum of last session_total per session', () => {
    ...
    assert.strictEqual(allDeltaInput, lastTotalA + lastTotalB); // 1600 == 1600
  });
});
...
describe('Scenario 3: first row in session (delta == session_total)', () => {
  test('no prior state → delta equals session_total', () => {
    ...
    assert.strictEqual(delta['input'], sessionTotal['input']);
    assert.strictEqual(delta['output'], sessionTotal['output']);
    assert.strictEqual(delta['cache_creation'], sessionTotal['cache_creation']);
    assert.strictEqual(delta['cache_read'], sessionTotal['cache_read']);
  });

  test('.session-totals.json is created on first fire', () => {
    ...
    assert.notStrictEqual(sessionTotals, null);
    assert.strictEqual(typeof sessionTotals![sessionId], 'object');
    const entry = sessionTotals![sessionId] as Record<string, unknown>;
    assert.strictEqual(entry['input'], 100);
    assert.strictEqual(entry['output'], 50);
  });
});
...
describe('Scenario 4: Reporter on new-format ledger (sumDeltas)', () => {
  test('delta-format ledger sums correctly via sumDeltas', () => {
    const rows = [
      {
    ...
    assert.strictEqual(result.format, 'delta');
    assert.strictEqual(result.totals.input, 371);           // 271 + 100
    assert.strictEqual(result.totals.output, 19107);        // 18607 + 500
    assert.strictEqual(result.totals.cache_creation, 79726); // 78726 + 1000
    assert.strictEqual(result.totals.cache_read, 1706009);  // 1701009 + 5000
    assert.strictEqual(result.pre_v2_caveat, undefined);
  });
});
...
describe('Scenario 5: Reporter on pre-0.9.0 ledger (flat fields)', () => {
  test('flat-field ledger uses last-row trick and returns format caveat', () => {
    ...
    assert.strictEqual(result.format, 'pre-0.9.0');
    // Last row for legacy-session-1 → input=2400
    assert.strictEqual(result.totals.input, 2400);
    assert.strictEqual(result.totals.output, 18600);
    assert.notStrictEqual(result.pre_v2_caveat, undefined);
    assert.ok(String(result.pre_v2_caveat).includes('pre-0.9.0 flat-field rows'));
    assert.ok(String(result.pre_v2_caveat).includes('last-row-per-session trick'));
  });
});
...
describe('Scenario 6: concurrent hook fires — atomic state write', () => {
  test('two parallel hook fires produce valid, non-corrupt .session-totals.json', { timeout: 30000 }, async () => {
    ...
    assert.notStrictEqual(parsedTotals, null);
    ...
    assert.strictEqual(hasA || hasB, true);
  });
});
...
describe('Scenario 7: mixed-format ledger (SPRINT-15 cutover case)', () => {
  test('returns format: mixed with caveat and correct math', () => {
    const rows = [
    ...
    assert.strictEqual(result.format, 'mixed');
    // flat legacy-session last row: input=2000; delta-session: input=300 → total=2300
    assert.strictEqual(result.totals.input, 2300);
    assert.strictEqual(result.totals.output, 4600);       // 4000 + 600
    assert.strictEqual(result.totals.cache_creation, 6900); // 6000 + 900
    assert.strictEqual(result.totals.cache_read, 9200);   // 8000 + 1200
    assert.notStrictEqual(result.pre_v2_caveat, undefined);
    assert.match(String(result.pre_v2_caveat), /Mixed format ledger/);
    assert.match(String(result.pre_v2_caveat), /1 delta rows/);
    assert.match(String(result.pre_v2_caveat), /2 pre-0\.9\.0 rows/);
    assert.match(String(result.pre_v2_caveat), /last-row trick/);
  });
});
