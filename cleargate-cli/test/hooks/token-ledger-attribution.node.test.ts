...
describe('token-ledger.sh: dispatch-marker attribution (CR-016)', () => {
   ...
   */
  test('scenario 1: dispatch file present → work_item_id and agent_type used verbatim', () => {
    ...
    assert.strictEqual(row!['work_item_id'], 'CR-016');
   ...
   */
  test('scenario 2: dispatch file absent → fallback to transcript scan, no exception', () => {
    ...
    assert.strictEqual(row!['work_item_id'], 'STORY-015-06');
  });
   ...
   */
  test('scenario 3: dispatch file malformed JSON → fallback + warning logged, no crash', () => {
   ...
   */
  test('scenario 4: dispatch wins over transcript pollution', () => {
    ...
    assert.strictEqual(row!['work_item_id'], 'CR-016');
    assert.strictEqual(row!['agent_type'], 'developer');
    assert.strictEqual(row!['story_id'], ''); // CR-016 is not STORY-*
  });
   ...
   */
  test('scenario 5: dispatch file is consumed and deleted after successful row write', () => {
    ...
    assert.notStrictEqual(row, null);
    assert.strictEqual(row!['work_item_id'], 'STORY-020-02');
    assert.strictEqual(row!['agent_type'], 'developer');
    assert.strictEqual(row!['story_id'], 'STORY-020-02');
    ...
    assert.strictEqual((processedFiles).length, 0);
  });
   ...
   */
  test('STORY-* dispatch: story_id derived from work_item_id', () => {
    ...
    assert.notStrictEqual(row, null);
    assert.strictEqual(row!['work_item_id'], 'STORY-015-06');
    assert.strictEqual(row!['agent_type'], 'qa');
    assert.strictEqual(row!['story_id'], 'STORY-015-06'); // STORY-* → story_id populated
  });
   ...
   */
  test('dispatch JSON valid but missing agent_type field → fallback + no crash', () => {
   ...
   */
  test('CR-026-A: dispatch with non-session-id filename → newest-file lookup attributes correctly', () => {
    ...
    assert.notStrictEqual(row, null);
    assert.strictEqual(row!['work_item_id'], 'CR-026');
    assert.strictEqual(row!['agent_type'], 'developer');
  });
   ...
   */
  test('CR-026-B: SessionStart-poisoned transcript → banner skipped, real work-item attributed', () => {
    ...
    assert.notStrictEqual(row!['work_item_id'], 'BUG-004');
    // Must pick up STORY-026-01 from the second user message
    assert.strictEqual(row!['work_item_id'], 'STORY-026-01');
  });
});
