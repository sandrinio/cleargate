...
describe('stamp-and-gate.sh hook', () => {
  test('logs stamp=0 gate=0 ingest=0 for a valid .cleargate/delivery path', () => {
    ...
    assert.ok(String(logContent).includes('stamp=0'));
    assert.ok(String(logContent).includes('gate=0'));
    assert.ok(String(logContent).includes('ingest=0'));
    assert.ok(String(logContent).includes('TEST-001.md'));
  });

  test('exits 0 and writes no log line when path is NOT under .cleargate/delivery/', () => {
    const tmpDir = makeTmpDir();
    ...
    assert.ok(!String(logContent).includes('file=/tmp/some-other-file.md'));
  });

  test('exits 0 even when gate check returns non-zero, and logs gate=<nonzero>', () => {
    const tmpDir = makeTmpDir();
    ...
    assert.strictEqual(exitCode, 0);
    assert.ok(String(logContent).includes('gate=1'));
    assert.ok(String(logContent).includes('stamp=0'));
    assert.ok(String(logContent).includes('ingest=0'));
  });
});
