...
function assertScaffoldShape(tmp: string): void {
  ...
  assert.strictEqual(typeof manifest['cleargate_version'], '.install-manifest.json cleargate_version field', 'string');
...
describe('Fixture A: Blank Node', () => {
  test('Scenario 1: Init into blank Node repo succeeds', async () => {
  ...
  test('Scenario 3: scaffold-lint on installed Node scaffold is clean', async () => {
    const tmp = buildNodeFixture();
    try {
      await runInit(tmp);
  ...
  test('Scenario 4: Gate friendly-fallback in fresh fixture (no config.yml)', async () => {
    const tmp = buildNodeFixture();
    try {
      await runInit(tmp);
  ...
  test('Scenario 5: Configured gate runs the command', async () => {
    const tmp = buildNodeFixture();
    try {
      await runInit(tmp);
  ...
  test('Scenario 6: Wiki build works in fixture', async () => {
    const tmp = buildNodeFixture();
    try {
      await runInit(tmp);
      ...
      assert.ok(String(indexContent).includes('PROPOSAL-999'));
    } finally {
      cleanup(tmp);
    }
  }, TEST_TIMEOUT_MS);
...
describe('Fixture B: Bare Go', () => {
  test('Scenario 2: Init into bare Go repo succeeds', async () => {
