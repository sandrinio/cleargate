...
describe('Integration: cache → wiki overlay', () => {
  ...
  test('seeds cache with 3 comments, renders wiki section with banner and all comment blocks', async () => {
    ...
    assert.notStrictEqual(commentHeaders, null);
    assert.strictEqual(commentHeaders?.length, 3);

    // Comment bodies
    assert.ok(String(result).includes('> First review comment'));
    assert.ok(String(result).includes('> A comment without email'));
    // Multi-line body: each line prefixed with "> "
    assert.ok(String(result).includes('> Multi-line'));
    assert.ok(String(result).includes('> body comment'));
    assert.ok(String(result).includes('> here'));

    // Author without email: no parens around null
    assert.ok(String(result).includes('### Bob · 2026-04-16T10:00:00Z'));
    assert.ok(!String(result).includes('(null)'));

    // Original content preserved
    assert.ok(String(result).includes('Initial page content.'));
    ...
    assert.ok(aliceIdx < charlieIdx);
  });

  test('cache returns null for absent file', async () => {
