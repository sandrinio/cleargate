...
describe('Scenario 1 (Gherkin): Active/Archive split', () => {
  ...
  test('index.md contains "## Active" before "## Archive"', async () => {
    ...
    assert.ok(activePos >= 0);
    assert.ok(archivePos >= 0);
    assert.ok(activePos < archivePos);
  });

  test('Active section lists only the 8 non-terminal epics by ID', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    ...
    for (const id of ['EPIC-008', 'EPIC-009', 'EPIC-010', 'EPIC-011', 'EPIC-012', 'EPIC-013', 'EPIC-014', 'EPIC-015']) {
      assert.ok(String(activeSection, `Expected ${id} in Active section`).includes(id));
    }
    ...
    for (const id of ['EPIC-001', 'EPIC-002', 'EPIC-003', 'EPIC-004', 'EPIC-005', 'EPIC-006', 'EPIC-007']) {
      assert.ok(!String(activeSection, `${id} should NOT be in Active section`).includes(id));
    }
  });

  test('Archive section contains "Epics: 7 Completed"', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const archiveSection = index.slice(index.indexOf('## Archive'));
    assert.ok(String(archiveSection).includes('7 Completed'));
  });

  test('Completed epics do not appear as individual rows', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    // Individual rows would look like [[EPIC-001]]; they must not appear anywhere
    for (const id of ['EPIC-001', 'EPIC-002', 'EPIC-003']) {
      assert.ok(!String(index).includes(`[[${id}]]`));
    }
  });
});
...
describe('Scenario 2 (Gherkin): Story rollup under epic (n=10)', () => {
  ...
  test('rollup line "STORY-014-xx (10 stories) — 10 Ready" appears under EPIC-014', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    assert.ok(String(index).includes('STORY-014-xx (10 stories) — 10 Ready'));
  });

  test('no individual [[STORY-014-xx]] row appears in Active', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const activeSection = index.slice(index.indexOf('## Active'), index.indexOf('## Archive'));
    ...
    for (let i = 1; i <= 10; i++) {
      const storyId = `STORY-014-${String(i).padStart(2, '0')}`;
      assert.ok(!String(activeSection, `${storyId} should be rolled up, not individual`).includes(`[[${storyId}]]`));
    }
  });

  test('EPIC-014 itself still appears in Active', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const activeSection = index.slice(index.indexOf('## Active'), index.indexOf('## Archive'));
    assert.ok(String(activeSection).includes('EPIC-014'));
  });
});
...
describe('Scenario 2b (Boundary): Exactly 3 stories under epic triggers rollup', () => {
  ...
  test('exactly 3 stories collapses to rollup line (>= threshold)', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    assert.ok(String(index).includes('STORY-020-xx (3 stories)'));
  });

  test('no individual [[STORY-020-xx]] rows in Active when exactly 3', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const activeSection = index.slice(index.indexOf('## Active'), index.indexOf('## Archive'));
    assert.ok(!String(activeSection).includes('[[STORY-020-01]]'));
    assert.ok(!String(activeSection).includes('[[STORY-020-02]]'));
    assert.ok(!String(activeSection).includes('[[STORY-020-03]]'));
  });

  test('exactly 2 stories does NOT collapse (shows individual rows)', async () => {
    ...
    try {
      ...
      const activeSection = index.slice(index.indexOf('## Active'), index.indexOf('## Archive'));
      assert.ok(String(activeSection).includes('[[STORY-021-01]]'));
      assert.ok(String(activeSection).includes('[[STORY-021-02]]'));
      assert.ok(!String(activeSection).includes('STORY-021-xx'));
    } finally {
      f2.cleanup();
    }
  });
...
describe('Scenario 3 (Gherkin): Orphan story keeps individual row', () => {
  ...
  test('[[STORY-999-00]] appears as its own row in Active', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const activeSection = index.slice(index.indexOf('## Active'), index.indexOf('## Archive'));
    assert.ok(String(activeSection).includes('[[STORY-999-00]]'));
  });

  test('orphan story is NOT in Archive section', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const archiveSection = index.slice(index.indexOf('## Archive'));
    ...
    assert.ok(!String(archiveSection).includes('[[STORY-999-00]]'));
  });
});
...
describe('Scenario 3b: Story whose parent epic is Completed becomes orphan', () => {
  ...
  test('story with archived-epic parent appears as individual row in Active', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const activeSection = index.slice(index.indexOf('## Active'), index.indexOf('## Archive'));
    assert.ok(String(activeSection).includes('[[STORY-002-01]]'));
  });
});
...
describe('Scenario 4 (Gherkin): Archive collapsed to summary', () => {
  ...
  test('Archive section contains "7 Completed" for epics', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const archiveSection = index.slice(index.indexOf('## Archive'));
    assert.ok(String(archiveSection).includes('7 Completed'));
  ...
  test('Archive section contains "2 Abandoned" for epics', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const archiveSection = index.slice(index.indexOf('## Archive'));
    assert.ok(String(archiveSection).includes('2 Abandoned'));
  });

  test('Archive epic summary is on a single line "Epics: 7 Completed · 2 Abandoned"', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    // The line order: 7 Completed appears before 2 Abandoned (sorted by count desc)
    const archiveSection = index.slice(index.indexOf('## Archive'));
    assert.ok(String(archiveSection).includes('Epics: 7 Completed · 2 Abandoned'));
  });

  test('no individual [[EPIC-xxx]] rows appear in the rendered index', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    for (let i = 1; i <= 9; i++) {
      const id = `EPIC-${String(i).padStart(3, '0')}`;
      assert.ok(!String(index, `${id} must not appear as individual [[wikilink]]`).includes(`[[${id}]]`));
    }
  });
});
...
describe('Scenario 5 (Gherkin): Idempotent rebuild', () => {
  ...
  test('two builds with frozen now produce byte-identical index.md', async () => {
    await runBuild(fixture);
  ...
  test('two builds produce byte-identical index.md (SHA comparison)', async () => {
    await runBuild(fixture);
...
describe('golden-file: matches committed expected-index-hierarchical.md', () => {
  ...
  test('golden-file: matches committed expected-index-hierarchical.md', async () => {
    await runBuild(fixture);
...
describe('Additional: Empty delivery uses hierarchical empty-state format', () => {
  ...
  test('index.md contains "## Active" and "## Archive" even when empty', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    assert.ok(String(index).includes('## Active'));
    assert.ok(String(index).includes('## Archive'));
  });

  test('index.md contains "_No active items._" placeholder', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    assert.ok(String(index).includes('_No active items._'));
  });

  test('index.md contains "_No archived items._" placeholder', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    assert.ok(String(index).includes('_No archived items._'));
  });
});
...
describe('Additional: Rollup status breakdown sorted by count desc', () => {
  ...
  test('status breakdown shows "5 Ready · 3 Draft" (count desc)', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    assert.ok(String(index).includes('STORY-030-xx (8 stories) — 5 Ready · 3 Draft'));
  });
});
...
describe('Additional: "Done" status is treated as terminal (alias)', () => {
  ...
  test('EPIC-040 with status Done goes to Archive, not Active', async () => {
    await runBuild(fixture);
    const index = readIndex(fixture);
    const activeSection = index.slice(index.indexOf('## Active'), index.indexOf('## Archive'));
    assert.ok(!String(activeSection).includes('EPIC-040'));
    const archiveSection = index.slice(index.indexOf('## Archive'));
    assert.ok(String(archiveSection).includes('1 Done'));
  });
});
