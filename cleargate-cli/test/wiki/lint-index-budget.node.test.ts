...
describe('Scenario 1: Within ceiling passes silently', () => {
  ...
  test('exits 0 when index tokens are under default ceiling', async () => {
    const result = await runLint(fixture);
    assert.strictEqual(result.exitCode, 0);
  });

  test('does not emit index-budget: error when under ceiling', async () => {
    const result = await runLint(fixture);
...
describe('Scenario 2: Over ceiling fails', () => {
  ...
  test('exits non-zero when index tokens exceed default ceiling', async () => {
    const result = await runLint(fixture);
    assert.strictEqual(result.exitCode, 1);
  });

  test('emits exact error message matching Gherkin spec', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes(
      'index-budget: wiki/index.md exceeds token ceiling: 9000 > 8000. Shard or prune (see EPIC-015).',
    ));
  });

  test('emits lint: FAIL summary', async () => {
    const result = await runLint(fixture);
...
describe('Scenario 3: Custom ceiling from config', () => {
  ...
  test('exits non-zero when over custom ceiling', async () => {
    const result = await runLint(fixture);
    assert.strictEqual(result.exitCode, 1);
  });

  test('references custom ceiling 4000 in error message', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('5000 > 4000'));
  });

  test('emits full message with custom ceiling', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes(
...
describe('Scenario 4: --suggest never fails', () => {
  ...
  test('exits 0 in suggest mode even when over ceiling', async () => {
    const result = await runLint(fixture, { mode: 'suggest' });
    assert.strictEqual(result.exitCode, 0);
  });

  test('emits usage line with correct tokens/ceiling/pct', async () => {
    const result = await runLint(fixture, { mode: 'suggest' });
    // 9000 / 8000 = 1.125 → Math.round → 113%
    assert.ok(String(result.stdout).includes('index token usage: 9000 / 8000 (113%)'));
  });

  test('does NOT emit index-budget: finding line in suggest mode', async () => {
    const result = await runLint(fixture, { mode: 'suggest' });
  ...
  test('emits lint: OK summary in suggest mode', async () => {
    const result = await runLint(fixture, { mode: 'suggest' });
