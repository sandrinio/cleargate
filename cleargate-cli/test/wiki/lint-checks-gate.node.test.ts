...
describe('Scenario: Enforcing fail — Epic with status:Ready + pass:false', () => {
  ...
  test('checkGateFailure emits gate-failure finding with criterion id', () => {
...
`;
    ...
    assert.strictEqual(finding!.category, 'gate-failure');
    assert.ok(String(finding!.line).includes('EPIC-Z'));
    assert.ok(String(finding!.line).includes('no-tbds'));
  });
});
...
describe('Scenario: Advisory tolerated — Proposal with pass:false is not enforcing', () => {
  ...
  test('checkGateFailure returns null for Proposal type (advisory only)', () => {
    const rawContent = `---
proposal_id: "PROP-Z"
status: "Ready"
ambiguity: "🟢 Low"
...
describe('Scenario: Null pass ignored — Epic with pass:null returns null', () => {
  ...
  test('checkGateFailure returns null when cached_gate_result.pass is null', () => {
...
describe('Scenario: Staleness — last_gate_check < updated_at emits gate-stale', () => {
  ...
  test('checkGateStaleness emits gate-stale when last_gate_check is before updated_at', () => {
    const rawContent = `---
epic_id: "EPIC-Z"
status: "Ready"
updated_at: "2026-04-19T00:00:00Z"
...
`;
    ...
    assert.notStrictEqual(finding, null);
    assert.strictEqual(finding!.category, 'gate-stale');
    assert.ok(String(finding!.line).includes('last_gate_check=2026-04-18T00:00:00Z'));
    assert.ok(String(finding!.line).includes('updated_at=2026-04-19T00:00:00Z'));
  });
});
...
describe('Scenario: Fresh — last_gate_check >= updated_at returns null', () => {
  ...
  test('checkGateStaleness returns null when last_gate_check is equal to updated_at', () => {
    const rawContent = `---
epic_id: "EPIC-Z"
status: "Ready"
updated_at: "2026-04-19T00:00:00Z"
...
`;
  ...
  test('checkGateStaleness returns null when last_gate_check is after updated_at', () => {
    const rawContent = `---
epic_id: "EPIC-Z"
status: "Ready"
