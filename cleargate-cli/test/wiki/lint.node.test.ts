...
describe('Scenario 1: All-clean wiki: exit 0, lint: OK', () => {
  ...
  test('stdout contains lint: OK', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('lint: OK'));
  });

  test('stdout contains "(8 pages checked, 0 findings)"', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('8 pages checked, 0 findings'));
  });
});
...
describe('Scenario 2: Orphan detected', () => {
  ...
  test('exits 1 in enforce mode', async () => {
    const result = await runLint(fixture);
    assert.strictEqual(result.exitCode, 1);
  });

  test('stdout contains orphan: category string', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('orphan:'));
  });

  test('stdout contains the raw_path in the finding', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('.cleargate/delivery/pending-sync/EPIC-FOO.md'));
  });

  test('stdout contains "lint: FAIL"', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('lint: FAIL'));
  });
});
...
describe('Scenario 3: Repo-mismatch detected', () => {
  ...
  test('stdout contains repo-mismatch: category string', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('repo-mismatch:'));
  });

  test('stdout mentions both stored and derived repo values', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('repo:cli'));
    assert.ok(String(result.stdout).includes('repo:planning'));
  });
});
...
describe('Scenario 4: Stale-commit detected', () => {
  ...
  test('exits 1 when runner returns different SHA', async () => {
    const result = await runLint(fixture, {
  ...
  test('stdout contains stale-commit: category string', async () => {
    const result = await runLint(fixture, {
      gitRunner: (_cmd, _args) => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('stale-commit:'));
  });

  test('stdout mentions stored SHA', async () => {
    const result = await runLint(fixture, {
      gitRunner: (_cmd, _args) => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes(STORED_SHA));
  });

  test('exits 0 when runner returns same SHA (no stale commit)', async () => {
    const result = await runLint(fixture, {
...
describe('Scenario 5: Missing-ingest detected via mtime', () => {
  ...
  test('stdout contains missing-ingest: category string', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('missing-ingest:'));
  });
});
...
describe('Scenario 6: Broken-backlink detected', () => {
  ...
  test('stdout contains broken-backlink: category string', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('broken-backlink:'));
  });

  test('stdout mentions the child page', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('STORY-010-01'));
  });
});
...
describe('Scenario 7: Invalidated-citation detected', () => {
  ...
  test('stdout contains invalidated-citation: category string', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('invalidated-citation:'));
  });
});
...
describe('Scenario 8: Excluded-path-ingested detected', () => {
  ...
  test('stdout contains excluded-path-ingested: category string', async () => {
    const result = await runLint(fixture);
    assert.ok(String(result.stdout).includes('excluded-path-ingested:'));
  });
});
...
describe('Scenario 9: Suggest mode', () => {
  ...
  test('exits 0 even with findings', async () => {
    const result = await runLint(fixture, { mode: 'suggest' });
    assert.strictEqual(result.exitCode, 0);
  });

  test('flags are prefixed with [advisory]', async () => {
    const result = await runLint(fixture, { mode: 'suggest' });
    assert.ok(String(result.stdout).includes('[advisory]'));
    assert.ok(String(result.stdout).includes('[advisory] orphan:'));
  });

  test('summary still shows OK in suggest mode', async () => {
    const result = await runLint(fixture, { mode: 'suggest' });
    assert.ok(String(result.stdout).includes('lint: OK'));
  });
});
...
describe('Scenario 10: Literal-substring contract test — all 9 category strings', () => {
  ...
  test('stdout contains literal "orphan:" string', async () => {
    const result = await runLint(fixture, {
  ...
  test('stdout contains literal "repo-mismatch:" string', async () => {
    const result = await runLint(fixture, {
      gitRunner: () => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('repo-mismatch:'));
  ...
  test('stdout contains literal "stale-commit:" string', async () => {
    const result = await runLint(fixture, {
      gitRunner: () => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('stale-commit:'));
  ...
  test('stdout contains literal "missing-ingest:" string', async () => {
    const result = await runLint(fixture, {
      gitRunner: () => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('missing-ingest:'));
  ...
  test('stdout contains literal "broken-backlink:" string', async () => {
    const result = await runLint(fixture, {
      gitRunner: () => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('broken-backlink:'));
  ...
  test('stdout contains literal "invalidated-citation:" string', async () => {
    const result = await runLint(fixture, {
      gitRunner: () => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('invalidated-citation:'));
  ...
  test('stdout contains literal "excluded-path-ingested:" string', async () => {
    const result = await runLint(fixture, {
      gitRunner: () => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('excluded-path-ingested:'));
  ...
  test('stdout contains literal "pagination-needed:" string', async () => {
    const result = await runLint(fixture, {
      gitRunner: () => CURRENT_SHA + '\n',
    });
    assert.ok(String(result.stdout).includes('pagination-needed:'));
  });

  test('stdout contains literal "index-budget:" string (STORY-015-03, 9th category)', async () => {
    ...
    assert.ok(String(result.stdout).includes('index-budget:'));
  });
});
