...
describe('merge-helper promptThreeWayMerge()', () => {
  test('Scenario: merge helper keep-mine — stdin k\\n → resolution keep, body === local', async () => {
    ...
    assert.strictEqual(result.resolution, 'keep');
    assert.strictEqual(result.body, LOCAL);
  });

  test('Scenario: merge helper take-theirs — stdin t\\n → resolution take, body === remote', async () => {
    ...
    assert.strictEqual(result.resolution, 'take');
    assert.strictEqual(result.body, REMOTE);
  });

  test('Scenario: merge helper edit — editor writes resolved content, temp file unlinked', async () => {
    ...
    try {
      ...
      assert.strictEqual(result.resolution, 'edited');
      assert.strictEqual(result.body, resolvedBody);

      // Temp file must be cleaned up
      assert.strictEqual(fs.existsSync(capturedTmpFile), false);
    } finally {
  ...
  test('Scenario: merge helper abort — stdin a\\n → resolution aborted, no changes written', async () => {
    ...
    assert.strictEqual(result.resolution, 'aborted');
    assert.strictEqual(result.body, LOCAL);
  });

  test('Scenario: merge helper abort via Ctrl-C (stream close without newline) → resolution aborted', async () => {
