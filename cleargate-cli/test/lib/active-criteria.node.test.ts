...
describe('resolveActiveItems', () => {
  ...
  test('includes item referenced in the active sprint body', async () => {
    ...
    assert.strictEqual(active.has('LIN-1042'), true);
  });

  test('includes item with last_remote_update within 30 days even if not in sprint', async () => {
    ...
    assert.strictEqual(active.has('LIN-3304'), true);
  });

  test('excludes item not in sprint and older than 30 days', async () => {
    ...
    assert.strictEqual(active.has('LIN-3304'), false);
  });

  test('skips items with no remoteId', async () => {
