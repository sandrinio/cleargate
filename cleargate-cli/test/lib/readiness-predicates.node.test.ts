...
describe('parsePredicate — accepts all 6 shapes', () => {
  test('frontmatter(.) shape', () => {
    ...
    assert.strictEqual(p.kind, 'frontmatter');
    if (p.kind === 'frontmatter') {
      assert.strictEqual(p.ref, '.');
      assert.strictEqual(p.field, 'approved');
      assert.strictEqual(p.op, '==');
      assert.strictEqual(p.value, true);
    }
  });

  test('frontmatter(context_source) shape', () => {
    const p = parsePredicate('frontmatter(context_source).approved == true');
    assert.strictEqual(p.kind, 'frontmatter');
    if (p.kind === 'frontmatter') {
      assert.strictEqual(p.ref, 'context_source');
      assert.strictEqual(p.field, 'approved');
    }
  });

  test('body-contains shape', () => {
    const p = parsePredicate("body contains 'TBD'");
    assert.strictEqual(p.kind, 'body-contains');
    if (p.kind === 'body-contains') {
      assert.strictEqual(p.needle, 'TBD');
      assert.strictEqual(p.negated, false);
    }
  });

  test('body-does-not-contain shape', () => {
    const p = parsePredicate("body does not contain 'TBD'");
    assert.strictEqual(p.kind, 'body-contains');
    if (p.kind === 'body-contains') {
      assert.strictEqual(p.needle, 'TBD');
      assert.strictEqual(p.negated, true);
    }
  });

  test('section shape with ≥', () => {
    const p = parsePredicate('section(2) has ≥1 checked-checkbox');
    assert.strictEqual(p.kind, 'section');
    if (p.kind === 'section') {
      assert.strictEqual(p.index, 2);
      assert.strictEqual(p.count.op, '>=');
      assert.strictEqual(p.count.n, 1);
      assert.strictEqual(p.itemType, 'checked-checkbox');
    }
  });
  ...
  test('section shape with == ', () => {
    ...
    if (p.kind === 'section') {
      assert.strictEqual(p.count.op, '==');
      assert.strictEqual(p.count.n, 0);
      assert.strictEqual(p.itemType, 'listed-item');
    }
  });

  test('file-exists shape', () => {
    const p = parsePredicate('file-exists(.cleargate/knowledge/readiness-gates.md)');
    assert.strictEqual(p.kind, 'file-exists');
    if (p.kind === 'file-exists') {
      assert.strictEqual(p.path, '.cleargate/knowledge/readiness-gates.md');
    }
  });

  test('link-target-exists shape', () => {
    const p = parsePredicate('link-target-exists([[STORY-003-13]])');
    assert.strictEqual(p.kind, 'link-target-exists');
    if (p.kind === 'link-target-exists') {
      assert.strictEqual(p.id, 'STORY-003-13');
    }
  });

  test('status-of shape', () => {
    const p = parsePredicate('status-of([[EPIC-008]]) == Active');
    assert.strictEqual(p.kind, 'status-of');
    if (p.kind === 'status-of') {
      assert.strictEqual(p.id, 'EPIC-008');
      assert.strictEqual(p.value, 'Active');
    }
  });
});
...
describe('evaluate — frontmatter predicate', () => {
  ...
  test('frontmatter(context_source) → fail when linked file missing', () => {
    const dir = makeTmpDir();
    const docPath = path.join(dir, 'EPIC-001.md');
...
describe('evaluate — body-contains predicate', () => {
  test("body contains 'TBD' → pass when TBD present", () => {
    const doc = makeDoc({}, '## Section 1\n\nSome TBD content here.\n');
    const result = evaluate("body contains 'TBD'", doc);
    assert.strictEqual(result.pass, true);
    assert.match(String(result.detail), /found/);
  });

  test("body does not contain 'TBD' → fail when TBD present", () => {
    ...
    assert.match(String(result.detail), /2 occurrence/);
    assert.match(String(result.detail), /§/);
  });

  test("body does not contain 'TBD' → pass when TBD absent", () => {
    const doc = makeDoc({}, '## Section 1\n\nAll good.\n');
    const result = evaluate("body does not contain 'TBD'", doc);
    assert.strictEqual(result.pass, true);
  });

  test("body contains 'STORY-' → fail when absent", () => {
...
describe('evaluate — section predicate', () => {
  ...
  test('section(2) has ≥1 checked-checkbox → pass', () => {
...
`;
  ...
  test('section(2) has ≥1 checked-checkbox → fail when no checkboxes', () => {
    const body = `## Section 1
...
`;
  ...
  test('section(1) has ≥1 listed-item → pass', () => {
    const body = `## Section 1
...
`;
  ...
  test('section(3) has ≥3 listed-item → pass with exactly 3', () => {
    const body = `## Section 1
...
`;
  ...
  test('section(3) has ≥3 listed-item → fail with only 2', () => {
    const body = `## Section 1
...
`;
  ...
  test('section(99) → fail with "not found" detail', () => {
    const doc = makeDoc({}, '## Section 1\n\nContent.\n');
    const result = evaluate('section(99) has ≥1 listed-item', doc);
    assert.strictEqual(result.pass, false);
    assert.match(String(result.detail), /not found/);
  });
});
...
describe('evaluate — link-target-exists predicate', () => {
  ...
  test('link-target-exists → pass when ID in wiki index', () => {
    const dir = makeTmpDir();
  ...
  test('link-target-exists → fail when ID not in wiki index', () => {
    const dir = makeTmpDir();
    const wikiDir = path.join(dir, '.cleargate', 'wiki');
    fs.mkdirSync(wikiDir, { recursive: true });
    const wikiIndexPath = path.join(wikiDir, 'index.md');
  ...
  test('link-target-exists → fail when wiki index missing', () => {
    const dir = makeTmpDir();
...
describe('evaluate — status-of predicate', () => {
  test('status-of reads linked file frontmatter.status → pass', () => {
    const dir = makeTmpDir();
  ...
  test('status-of → fail when status does not match', () => {
    const dir = makeTmpDir();
    const deliveryDir = path.join(dir, '.cleargate', 'delivery', 'pending-sync');
    fs.mkdirSync(deliveryDir, { recursive: true });
    const epicFile = path.join(deliveryDir, 'EPIC-008.md');
    ...
    assert.strictEqual(result.pass, false);
    assert.match(String(result.detail), /Draft/);
  });
});
...
describe('sandbox: evaluator restrictions', () => {
  test('rejects FS reads outside projectRoot (path traversal)', () => {
    const dir = makeTmpDir();
    ...
    assert.match(String(result.detail), /not found|sandbox/i);
  });

  test('does not import child_process (sandbox: no shell-out)', async () => {
    ...
    assert.ok(!String(src).includes('child_process'));
    assert.ok(!String(src).includes('execSync'));
    assert.ok(!String(src).includes('spawnSync'));
  });
});
...
describe('CR-033 existing-surfaces-verified — L0 code-truth tightening', () => {
  ...
  test('Section absent → not-applicable (pass with skip detail)', () => {
...
`;
    ...
    assert.strictEqual(result.pass, true);
    assert.match(String(result.detail), /not-applicable/);
  });

  // Scenario 2: Section present, cites package.json (real top-level file) → pass.
  test('Section present, cites package.json (real top-level file) → pass', () => {
...
`;
    ...
    const result = evaluate('existing-surfaces-verified', doc, { projectRoot: SMOKE_REPO_ROOT });
    assert.strictEqual(result.pass, true);
    assert.match(String(result.detail), /exist/);
  });

  // Scenario 3: Section present, cites real file with :symbol suffix → pass; symbol stripped.
  test('Section present, cites cleargate-cli/src/lib/work-item-type.ts:detectWorkItemTypeFromFm → pass; symbol stripped', () => {
    const body = `## Existing Surfaces
...
`;
  ...
  test('Section present, cites cleargate-cli/src/lib/does-not-exist.ts → fail; detail names path', () => {
    const body = `## Existing Surfaces
...
`;
    ...
    const result = evaluate('existing-surfaces-verified', doc, { projectRoot: SMOKE_REPO_ROOT });
    assert.strictEqual(result.pass, false);
    assert.ok(String(result.detail).includes('cleargate-cli/src/lib/does-not-exist.ts'));
  });

  // Scenario 5: Section present, mix of real + missing → fail; detail names only missing.
  test('Section present, mix of real + missing paths → fail; detail names only missing', () => {
    const body = `## Existing Surfaces
...
`;
    ...
    assert.strictEqual(result.pass, false);
    assert.ok(String(result.detail).includes('cleargate-cli/src/lib/does-not-exist.ts'));
    assert.ok(!String(result.detail).includes('package.json'));
  });

  // Scenario 6: Section present, no path matches, contains "no overlap found" sentinel → pass.
  test('Section present, no path matches, contains "no overlap found" sentinel → pass', () => {
    const body = `## Existing Surfaces
...
`;
    ...
    const result = evaluate('existing-surfaces-verified', doc, { projectRoot: SMOKE_REPO_ROOT });
    assert.strictEqual(result.pass, true);
    assert.match(String(result.detail), /sentinel/);
  });

  // Scenario 7: Section present, no path matches, no sentinel → fail with sentinel-missing detail.
  test('Section present, no path matches, no sentinel → fail with sentinel-missing detail', () => {
    const body = `## Existing Surfaces
...
`;
    ...
    const result = evaluate('existing-surfaces-verified', doc, { projectRoot: SMOKE_REPO_ROOT });
    assert.strictEqual(result.pass, false);
    assert.ok(String(result.detail).includes('no path citations'));
    assert.ok(String(result.detail).includes('sentinel'));
  });
  ...
  test('Section present, cites path traversing outside root → sandbox-rejected, treated as missing → fail', () => {
    const body = `## Existing Surfaces
...
`;
    ...
    assert.match(String(result.detail), /cited paths do not exist/);
  });

  // Scenario 9: Section present, cites cleargate-cli/package.json (real subdir file) → pass.
  test('Section present, cites cleargate-cli/package.json (real subdir file) → pass', () => {
    const body = `## Existing Surfaces
...
describe('smoke test: readiness-gates.md parses correctly', () => {
  test('js-yaml parse of readiness-gates.md returns 7 blocks (6 original + sprint gate added by CR-027)', async () => {
    ...
    assert.strictEqual((yamlBlocks).length, 8);
    ...
    for (const block of parsed) {
      ...
      assert.ok('severity' in (block));
      assert.ok('criteria' in (block));
    }
  });

  test('every criterion.check parses via parsePredicate without throwing', async () => {
    const yaml = await import('js-yaml');
    const gatesPath = path.join(
      SMOKE_REPO_ROOT,
      '.cleargate/knowledge/readiness-gates.md'
  ...
  test('smoke: evaluate ≥1 real pending-sync item against proposal gate', () => {
...
describe('BUG-008 sub-fix #1 — parent-approved-proposal prose-vs-path heuristic', () => {
  test('R-08: context_source is prose with approved_by + approved_at → pass', () => {
    const dir = makeTmpDir();
    ...
    const result = evaluate('frontmatter(context_source).approved == true', doc, { projectRoot: dir });
    assert.strictEqual(result.pass, true);
    assert.match(String(result.detail), /prose.*waiver|waiver.*prose/i);
  });

  test('R-08: context_source is prose WITHOUT waiver fields → fail', () => {
    const dir = makeTmpDir();
    ...
    const result = evaluate('frontmatter(context_source).approved == true', doc, { projectRoot: dir });
    assert.strictEqual(result.pass, false);
    assert.match(String(result.detail), /no proposal_gate_waiver/i);
  });

  test('R-08 regression: context_source is "PROPOSAL-999.md" (no spaces, file does NOT exist) → fail even with waiver fields', () => {
    ...
    const result = evaluate('frontmatter(context_source).approved == true', doc, { projectRoot: dir });
    assert.strictEqual(result.pass, false);
    assert.match(String(result.detail), /linked file not found/i);
  });

  test('R-08 happy path: context_source is "PROPOSAL-001.md" and file exists with approved: true → pass', () => {
    const dir = makeTmpDir();
  ...
  test('R-08 sandbox: path traversal "../../../etc/passwd" does not pass via prose heuristic', () => {
...
describe('BUG-008 sub-fix #2 — no-tbds marker semantics', () => {
  test('body containing "TBD: <reason>" → fails (genuine marker with colon)', () => {
    const doc = makeDoc({}, '## Section 1\n\nTBD: figure out the architecture.\n');
    const result = evaluate("body does not contain marker 'TBD'", doc);
    assert.strictEqual(result.pass, false);
    assert.match(String(result.detail), /occurrence/);
  });

  test('body containing "(TBD)" → fails (parens-bound)', () => {
    const doc = makeDoc({}, '## Section 1\n\nThis feature is (TBD) pending review.\n');
    const result = evaluate("body does not contain marker 'TBD'", doc);
    assert.strictEqual(result.pass, false);
  });

  test('body containing bare line "TBD" → fails (entire trimmed line)', () => {
    const doc = makeDoc({}, '## Section 1\n\nTBD\n\nMore text.\n');
    const result = evaluate("body does not contain marker 'TBD'", doc);
    assert.strictEqual(result.pass, false);
  });

  test('body containing prose "TBD resolution" → passes (noun usage, not a marker)', () => {
  ...
  test('body containing "TBDs" (plural) → passes (not a marker)', () => {
    const doc = makeDoc({}, '## Section 1\n\nThe following TBDs were addressed in this sprint.\n');
    const result = evaluate("body does not contain marker 'TBD'", doc);
    assert.strictEqual(result.pass, true);
  });

  test('template self-reference line "- [x] 0 "TBDs" exist in the document." → passes (boilerplate excluded)', () => {
  ...
  test('body containing "TODO:" → fails (TODO marker with colon)', () => {
    const doc = makeDoc({}, '## Section 1\n\nTODO: implement this function\n');
    const result = evaluate("body does not contain marker 'TODO'", doc);
    assert.strictEqual(result.pass, false);
  });

  test('body containing "FIXME:" → fails (FIXME marker with colon)', () => {
...
describe('BUG-008 sub-fix #2 — parsePredicate marker-absence shape', () => {
  test('parses "body does not contain marker \'TBD\'" correctly', () => {
    const p = parsePredicate("body does not contain marker 'TBD'");
    assert.strictEqual(p.kind, 'marker-absence');
    if (p.kind === 'marker-absence') {
      assert.strictEqual(p.marker, 'TBD');
    }
  });

  test('parses "body does not contain marker \'TODO\'" correctly', () => {
    const p = parsePredicate("body does not contain marker 'TODO'");
    assert.strictEqual(p.kind, 'marker-absence');
    if (p.kind === 'marker-absence') {
      assert.strictEqual(p.marker, 'TODO');
    }
  });

  test('throws on unsupported marker "UNKNOWN"', () => {
...
describe('BUG-008 sub-fix #3 — blast-radius-populated section index', () => {
  test('CR fixture with §1 containing bullets AND §2 empty → fails (blast-radius targets §2)', () => {
...
`;
    ...
    assert.strictEqual(result.pass, false);
    assert.match(String(result.detail), /section 2/);
  });

  test('CR fixture with §2 containing ≥1 bullet → passes', () => {
    const body = `## 1. The Context Override (Old vs. New)
...
describe('CR-028 code-truth triage criteria', () => {
  ...
  test('Epic with ## Existing Surfaces AND ## Why not simpler? → reuse-audit-recorded + simplest-form-justified pass', () => {
    const body = `## 1. Problem & Value
...
`;
    ...
    assert.strictEqual(resultReuse.pass, true);

    const resultSimpler = evaluate("body contains '## Why not simpler?'", doc);
    assert.strictEqual(resultSimpler.pass, true);
  });

  // Scenario 2 — Epic missing ## Existing Surfaces → reuse-audit-recorded fails.
  test('Epic missing ## Existing Surfaces → reuse-audit-recorded fails with non-empty detail', () => {
    const body = `## 1. Problem & Value
...
`;
  ...
  test('Story missing ## Why not simpler? → simplest-form-justified fails', () => {
...
`;
  ...
  test('CR with ## Existing Surfaces present → reuse-audit-recorded passes; simplest-form-justified absent from cr.ready-to-apply', () => {
...
`;
    ...
    assert.notStrictEqual(crBlockMatch, null);
    const crBlock = crBlockMatch![0];
    assert.ok(!String(crBlock).includes('simplest-form-justified'));
  });
});
...
describe('CR-031: resolveLinkedPath — searches pending-sync and archive', () => {
  ...
  test('Scenario 1: citer in pending-sync, target in pending-sync resolves via candidate 1 (sibling)', () => {
    const dir = makeTmpDir();
  ...
  test('Scenario 2: citer in pending-sync, target in archive resolves via candidate 4', () => {
    const dir = makeTmpDir();
    const pendingSync = path.join(dir, '.cleargate', 'delivery', 'pending-sync');
    ...
    const result = evaluate('frontmatter(context_source).approved == true', doc, { projectRoot: dir });
    assert.strictEqual(result.pass, true);
    assert.doesNotMatch(String(result.detail), /linked file not found/i);
  });

  // Scenario 3: citer in archive, target in archive (frozen pair)
  test('Scenario 3: citer in archive, target in archive (frozen pair) resolves via candidate 1', () => {
    const dir = makeTmpDir();
    const archive = path.join(dir, '.cleargate', 'delivery', 'archive');
    fs.mkdirSync(archive, { recursive: true });
  ...
  test('Scenario 4: citer in archive, target in pending-sync resolves via candidate 3', () => {
    const dir = makeTmpDir();
    const pendingSync = path.join(dir, '.cleargate', 'delivery', 'pending-sync');
    const archive = path.join(dir, '.cleargate', 'delivery', 'archive');
    fs.mkdirSync(pendingSync, { recursive: true });
  ...
  test('Scenario 5: sandbox preserved — "../../../etc/passwd" traversal rejected', () => {
    const dir = makeTmpDir();
    const pendingSync = path.join(dir, '.cleargate', 'delivery', 'pending-sync');
    fs.mkdirSync(pendingSync, { recursive: true });
    ...
    assert.match(String(result.detail), /linked file not found|sandbox|prose.*waiver/i);
  });
});
...
describe('CR-034: declared-item item-type', () => {
  ...
  test('parsePredicate accepts declared-item item-type', () => {
    const p = parsePredicate('section(3) has ≥1 declared-item');
    assert.strictEqual(p.kind, 'section');
    if (p.kind === 'section') {
      assert.strictEqual(p.itemType, 'declared-item');
    }
  });

  // Scenario 1: section with 3 bullets passes declared-item
  test('Scenario 1: section with 3 bullets → declared-item count=3, passes ≥1', () => {
    const body = `## Section 1
...
`;
  ...
  test('Scenario 2: section with 4-row table → declared-item count=4, passes ≥1', () => {
    const body = `## Section 1
...
`;
  ...
  test('Scenario 3: section with 2 bullets + 3 table rows → declared-item count=5, passes ≥5', () => {
    const body = `## Section 1
...
`;
  ...
  test('Scenario 4: section with definition-list terms → passes ≥3 declared-item', () => {
    const body = `## Section 1
...
`;
  ...
  test('Scenario 5: empty section → fails ≥1 declared-item', () => {
    const body = `## Section 1
...
`;
  ...
  test('Scenario 6: table header + separator only (no data rows) → fails ≥1 declared-item', () => {
    const body = `## Section 1
...
`;
  ...
  test('Scenario 7: 1 bullet + 1 def-list + 2 table rows → passes ≥4 declared-item', () => {
    const body = `## Section 1
...
`;
  ...
  test('Scenario 8: dod-declared regression — listed-item with - [x] and - [ ] lines still passes', () => {
    const body = `## Section 1
...
`;
    ...
    const result = evaluate('section(2) has ≥1 listed-item', doc);
    assert.strictEqual(result.pass, true);
    assert.match(String(result.detail), /3 listed-item/);
  });
});
...
describe('BUG-026: validate_state.mjs exports validateShapeIgnoringVersion', () => {
  test('validateShapeIgnoringVersion is exported and callable — rejects invalid shape', async () => {
    ...
    assert.strictEqual(typeof mod.validateShapeIgnoringVersion, 'function');
  ...
  test('validateShapeIgnoringVersion is exported and callable — accepts minimal valid shape', async () => {
    const scriptPath = path.join(SMOKE_REPO_ROOT, '.cleargate', 'scripts', 'validate_state.mjs');
    const mod = await import(scriptPath);
    const validState = {
...
describe('CR-030 parent-approved per-type field map', () => {
  test('R-030-1: Epic with context_source → Proposal (approved: true) → frontmatter(context_source).approved == true passes', () => {
    const dir = makeTmpDir();
  ...
  test('R-030-2: Epic with context_source → Initiative (status: Triaged) → frontmatter(context_source).status == Triaged passes', () => {
    const dir = makeTmpDir();
  ...
  test('R-030-3: Initiative fixture passes advisory gate when §1 has ≥1 listed-item AND §5 has ≥1 listed-item AND no TBDs', () => {
    const dir = makeTmpDir();
    ...
    assert.strictEqual(noTbds.pass, true);
    assert.strictEqual(userFlow.pass, true);
    assert.strictEqual(successCriteria.pass, true);
  });
});
