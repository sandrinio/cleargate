...
describe('sumDeltas: delta-only input (post-0.9.0)', () => {
  test('sums delta.* across all rows when all rows have delta blocks', () => {
    const rows = [
    ...
    assert.strictEqual(result.totals.input, 300);       // 100+150+50
    assert.strictEqual(result.totals.output, 550);      // 200+250+100
    assert.strictEqual(result.totals.cache_creation, 800); // 300+350+150
    assert.strictEqual(result.totals.cache_read, 1050); // 400+450+200
    assert.strictEqual(result.pre_v2_caveat, undefined);
  });

  test('single row: totals equal that row delta', () => {
    const rows = [
    ...
    assert.strictEqual(result.format, 'delta');
    assert.strictEqual(result.totals.input, 271);
    assert.strictEqual(result.totals.output, 18607);
    assert.strictEqual(result.totals.cache_creation, 78726);
    assert.strictEqual(result.totals.cache_read, 1701009);
  });
});

describe('sumDeltas: flat-only input (pre-0.9.0)', () => {
  test('uses last-row-per-session trick: picks max ts row per session_id', () => {
    ...
    assert.strictEqual(result.totals.input, 2500);
    assert.strictEqual(result.totals.output, 5000);      // 4000 + 1000
    assert.strictEqual(result.totals.cache_creation, 7500); // 6000 + 1500
    assert.strictEqual(result.totals.cache_read, 10000); // 8000 + 2000
    assert.notStrictEqual(result.pre_v2_caveat, undefined);
    assert.match(String(result.pre_v2_caveat), /pre-0\.9\.0 flat-field rows/);
    assert.match(String(result.pre_v2_caveat), /last-row-per-session trick/);
  });

  test('single session with single row: totals equal that row flat fields', () => {
    const rows = [
    ...
    assert.strictEqual(result.format, 'pre-0.9.0');
    assert.strictEqual(result.totals.input, 800);
    assert.strictEqual(result.totals.output, 1600);
    assert.notStrictEqual(result.pre_v2_caveat, undefined);
  });

  test('reads from fixture file representing synthesized SPRINT-14 rows', async () => {
    ...
    assert.strictEqual(result.totals.input, 2400 + 500 + 1600 + 3100 + 600);  // = 8200
    assert.notStrictEqual(result.pre_v2_caveat, undefined);
  });
});

describe('sumDeltas: mixed-format input', () => {
  test('returns format: mixed, caveat includes both counts, totals combine delta + flat last-row trick', () => {
    ...
    assert.strictEqual(result.totals.input, 1650);          // 100 + 50 + 1500
    assert.strictEqual(result.totals.output, 2800);         // 200 + 100 + 2500
    assert.strictEqual(result.totals.cache_creation, 3950); // 300 + 150 + 3500
    assert.strictEqual(result.totals.cache_read, 5100);     // 400 + 200 + 4500
    assert.notStrictEqual(result.pre_v2_caveat, undefined);
    assert.match(String(result.pre_v2_caveat), /Mixed format ledger/);
    assert.match(String(result.pre_v2_caveat), /2 delta rows/);
...
describe('sumDeltas: edge cases', () => {
  test('empty array returns zero totals and format: delta', () => {
    const result = sumDeltas([]);
    assert.strictEqual(result.format, 'delta');
    assert.strictEqual(result.totals.input, 0);
    assert.strictEqual(result.totals.output, 0);
    assert.strictEqual(result.totals.cache_creation, 0);
    assert.strictEqual(result.totals.cache_read, 0);
    assert.strictEqual(result.pre_v2_caveat, undefined);
  });

  test('malformed rows (not objects) are skipped gracefully without throwing', () => {
    const rows = [
    ...
    assert.strictEqual(result!.totals.input, 100);
    assert.strictEqual(result!.format, 'delta');
  });

  test('row with partial delta (missing a field) is treated as flat row (not delta)', () => {
    ...
    assert.strictEqual(result.format, 'pre-0.9.0');
    assert.strictEqual(result.totals.input, 500); // flat fields used
  });
});
