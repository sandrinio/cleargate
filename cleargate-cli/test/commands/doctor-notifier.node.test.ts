...
describe('STORY-016-02: doctor --session-start update notifier', () => {
   ...
   */
  test('Notifier prints when latest > installed', async () => {
    ...
    assert.ok(String(output).includes('cleargate 0.9.0 available (current: 0.8.2)'));
    assert.ok(String(output).includes('run `cleargate upgrade` or see CHANGELOG'));
  });
   ...
   */
  test('No notifier when up to date', async () => {
    const { lines } = await runWithMockedCheck(
    ...
    const output = lines.join('\n');
    assert.ok(!String(output).includes('available'));
  });
   ...
   * Then stdout does NOT contain "available"
   */
  test('No notifier on opt-out', async () => {
    const { lines } = await runWithMockedCheck(
   ...
   */
  test('Notifier does not change exit code (outcome.blocker unmodified)', async () => {
    const dir = makeTmpDir();
   ...
   */
  test('Notifier fires for fresh install with no pending-sync/ dir', async () => {
    const { lines } = await runWithMockedCheck(
      { latest: '0.9.0', from: 'network' },
      '0.8.2'
    );
   ...
   */
  test('No notifier when checkLatestVersion returns from: "error" with null latest', async () => {
    const { lines } = await runWithMockedCheck(
