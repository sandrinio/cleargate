...
describe('adminLoginHandler stdout snapshot', () => {
   ...
   */
  test('L-snapshot: happy path stdout is byte-identical to pre-refactor output', async () => {
    ...
    // Exact snapshot — verify stdout contains expected lines.
    assert.ok(stdoutLines !== undefined); /* toMatchInlineSnapshot — verify manually */
    assert.ok(String(stdoutText).includes('Waiting for authorization...'));
    assert.ok(String(stdoutText).includes('Logged in successfully.'));
    assert.ok(String(stdoutText).includes('Credentials saved to'));
  });
});
