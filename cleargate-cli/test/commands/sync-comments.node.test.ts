...
describe('Scenario: Rate-limit skip is silent (R4 per-item 429)', () => {
  ...
  test('429 on one item does not halt sync; sync-log has skipped-rate-limit; other items still processed', async () => {
    ...
    assert.ok(String(commentCallsSucceeded).includes('LIN-4202'));
    ...
    assert.notStrictEqual(rateLimitEntry, undefined);
    assert.strictEqual(rateLimitEntry?.remote_id, 'LIN-4201');

    // Stdout shows sync completed (pulled 0, pushed 0)
    const combinedOutput = stdoutLines.join('');
    assert.ok(String(combinedOutput).includes('sync:'));
  });
});
