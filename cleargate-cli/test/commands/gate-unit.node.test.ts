...
describe('Scenario: Check passing Epic', () => {
  test('exit 0 and cached_gate_result.pass = true', async () => {
...
describe('Scenario: Check failing Story (enforcing)', () => {
  test('exit non-zero, stdout lists failing criteria, cached_gate_result.pass = false', async () => {
    ...
    assert.strictEqual(cached.pass, false);
...
describe('Scenario: Check failing Proposal (advisory)', () => {
  test('exit 0, stdout prefixes ⚠, cached_gate_result.pass = false', async () => {
    ...
    const combined = out.join('\n');
    assert.ok(String(combined).includes('\u26A0'));
    assert.ok(String(combined).includes('(advisory)'));
...
describe('Scenario: Verbose output', () => {
  test('stdout contains full predicate evaluation per failing criterion', async () => {
    ...
    assert.match(String(combined), /\[(pass|fail)\]/);
  });
});
...
describe('Scenario: Explain is read-only and cheap', () => {
  test('output ≤50 LLM-tokens and no predicate re-evaluation (frontmatter unchanged)', async () => {
...
describe('Scenario: Explicit transition override', () => {
  test('evaluation runs against the specified transition even when already passed', async () => {
    ...
    const combined = out.join('\n');
    assert.ok(String(combined).includes('ready-for-decomposition'));
  });
});
...
describe('explain with no cached result', () => {
  test('prints hint and exits 0', async () => {
    ...
    assert.strictEqual((exitCodes).length, 0);
    const combined = out.join('\n');
    assert.ok(String(combined).includes('no gate check cached'));
    assert.ok(String(combined).includes('cleargate gate check'));
  });
});
...
describe('check on file with no ID frontmatter', () => {
  test('errors with "unable to detect work-item type"', async () => {
    const noIdFile = `---
title: "No ID here"
status: "Draft"
---
...
describe('transition-inference', () => {
  test('Epic with no cached gate → picks ready-for-decomposition (first)', async () => {
    // Verify WORK_ITEM_TRANSITIONS maps correctly
    assert.strictEqual(WORK_ITEM_TRANSITIONS['epic'][0], 'ready-for-decomposition');
    assert.strictEqual(WORK_ITEM_TRANSITIONS['epic'][1], 'ready-for-coding');
  ...
  test('Epic already passed decomposition → picks ready-for-coding', async () => {
    const { dir, absPath, gatesDocPath } = setupEnv(PASSING_EPIC_CACHED_PASS);
    const out: string[] = [];
...
describe('BUG-008 smoke: readiness-gates.md after fix parses cleanly', () => {
  ...
  test('readiness-gates.md parses all gate blocks with no errors (CR-027 adds sprint=7th, CR-030 adds initiative=8th)', async () => {
  ...
  test('all criterion check strings in readiness-gates.md parse via parsePredicate without throwing', async () => {
  ...
  test('EPIC-021-style fixture: prose context_source with approved_by + approved_at passes parent-approved', async () => {
...
`;
    ...
    assert.doesNotMatch(String(combined), /❌ parent-approved/);
  });

  test('CR-010-style fixture: prose "TBD resolution" in body passes no-tbds marker predicate', async () => {
...
`;
    ...
    assert.doesNotMatch(String(combined), /❌ no-tbds/);
  });

  test('CR-011-style fixture: §2 Blast Radius with items passes blast-radius-populated', async () => {
...
`;
    ...
    assert.doesNotMatch(String(combined), /❌ blast-radius-populated/);
  });
});
...
describe('CR-030 γ: OR-group evaluator — parent-approved', () => {
  test('Epic with Proposal parent (approved: true, no status) → gate passes for parent-approved concern', async () => {
    const dir = makeTmpDir();
  ...
  test('Epic with Initiative parent (status: Triaged, no approved) → gate passes for parent-approved concern', async () => {
    const dir = makeTmpDir();
  ...
  test('Epic with no valid parent (file missing) → gate fails with OR-group-failed error', async () => {
    const dir = makeTmpDir();
    ...
    // Must have failed
    assert.ok(String(exitCodes).includes(1));

    const combined = out.join('\n');
    // Must emit the OR-group failure message
    assert.match(String(combined), /❌ parent-approved/);
    // The detail should describe the OR-group failure
    assert.match(String(combined), /OR-group failed/);
  });
});
