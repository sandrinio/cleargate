...
describe('Scenario: all five checks pass in clean state', () => {
  test('exit code is 0 and stdout contains "all five checks pass"', () => {
...
describe('Scenario: previous sprint not Completed', () => {
  test('exit code is 1, stderr names prev sprint and includes close hint', () => {
...
describe('Scenario: leftover worktree', () => {
  test('exit code is 1, stderr names worktree path and includes "git worktree remove" hint', () => {
...
describe('Scenario: sprint branch ref already exists', () => {
  test('exit code is 1, stderr contains "Sprint branch ref already exists: refs/heads/sprint/S-19"', () => {
    buildCleanFixture(fixtureDir, 'SPRINT-19', 'Completed');
...
describe('Scenario: main is dirty', () => {
  test('exit code is 1, stderr contains "main is dirty" and the dirty filename', () => {
    buildCleanFixture(fixtureDir, 'SPRINT-19', 'Completed');
...
describe('Scenario: multiple checks fail simultaneously', () => {
  test('exit code is 1, stderr contains both failures and shows all 4 checks ran', () => {
    ...
    assert.ok(String(errOut).includes('Previous sprint not Completed'));
    assert.ok(String(errOut).includes('Leftover worktree'));
    // Both pass (✓) and fail (✗) entries appear — all four ran
    assert.ok(String(errOut).includes('✓'));
    assert.ok(String(errOut).includes('✗'));
    // exactly 2 out of 5 checks failed
    assert.ok(String(errOut).includes('2/5 checks failed'));
  });
});
...
describe('Scenario: usage error on missing / malformed sprint-id arg', () => {
  test('exit code is 2 with usage hint for empty sprint-id', () => {
  ...
  test('exit code is 2 for malformed sprint-id like "SPRINT-X"', () => {
    const cap = makeCapture();
    const { exitFn, getCode } = makeExitSeam();
    try {
      sprintPreflightHandler(
...
describe('Scenario: skip prev-sprint check for SPRINT-01', () => {
  test('exit code is 0 in clean state (check 1 skipped, other four pass)', () => {
  ...
  test('when dirty, check 1 shows as skipped in the punch list', () => {
...
describe('Scenario (STORY-026-01): preflight success emits skill load directive', () => {
  test('stdout last line is "→ Load skill: sprint-execution" when all five checks pass', () => {
    buildCleanFixture(fixtureDir, 'SPRINT-18', 'Completed');
    ...
    assert.strictEqual(lines[lines.length - 1], '→ Load skill: sprint-execution');
  });
});

describe('Scenario (STORY-026-01): preflight failure stays quiet on skill directive', () => {
  test('stdout does NOT contain "Load skill: sprint-execution" when a check fails', () => {
...
describe('Scenario 9 (CR-027): v2 happy path — all items pass=true', () => {
  test('exit code is 0, "all five checks pass", no Per-item readiness gates failure', () => {
...
describe('Scenario 10 (CR-027): v2 hard-block — one EPIC pass=false', () => {
  test('exit code is 1, stderr names EPIC-099 and failing criterion no-tbds', () => {
    const execFn = seedReadinessFixture(
      fixtureDir,
      'SPRINT-99',
      'v2',
...
describe('Scenario 11 (CR-027): staleness — last_gate_check older than updated_at', () => {
  test('exit code is 1, stderr contains EPIC-099 and stale reason', () => {
    const execFn = seedReadinessFixture(
      fixtureDir,
      'SPRINT-99',
      'v2',
...
describe('Scenario 12 (CR-027): v1 mode — check #5 skipped, exit 0', () => {
  test('exit code is 0 even with one item pass=false under v1', () => {
...
describe('Scenario 13 (CR-027): Done items skipped from failure list', () => {
  test('exit code is 0 when only Done item has pass=false (terminal status skipped)', () => {
    const execFn = seedReadinessFixture(
      fixtureDir,
      'SPRINT-99',
      'v2',
...
describe('Scenario 14 (CR-027): sprint plan itself fails when cached_gate_result is null', () => {
  test('exit code is 1, stderr names SPRINT-99 as failing item with no cached_gate_result', () => {
...
describe('Scenario 15 (CR-038): Step 0 — all items refreshed, 0 errors', () => {
  test('stdout contains "Step 0: refreshed N items, 0 errors." and exit code 0', () => {
    ...
    assert.strictEqual(gateCheckCalls['CR-099'], 1);
    assert.strictEqual(gateCheckCalls['EPIC-099'], 1);
    assert.strictEqual(gateCheckCalls['STORY-099-01'], 1);
  });
});
...
describe('Scenario 16 (CR-038): Step 0 — stale items get gate check invoked', () => {
  test('Step 0 reports 2 refreshed items and execFn receives 2 gate-check calls', () => {
    ...
    assert.strictEqual(code, 1);  // Check 5 still sees stale because mock doesn't write frontmatter
  });
});
...
describe('Scenario 17 (CR-038): Step 0 — per-item gate check error, other items still refreshed', () => {
  test('Step 0 reports N-1 refreshed + 1 error; preflight continues (exit driven by Check 5)', () => {
    const { execFn, gateCheckCalls } = seedStep0Fixture(fixtureDir, 'SPRINT-99', [
    ...
    assert.ok(String(out).includes('Step 0: refreshed 2 items, 1 errors.'));
    // Error bullet line for the failing item
    assert.ok(String(out).includes('CR-099'));
...
describe('Scenario 18 (CR-038): Step 0 — archive/terminal item skipped (no gate check)', () => {
  test('Step 0 skips Done item in archive; gate check not called for it', () => {
    const { execFn, gateCheckCalls } = seedStep0Fixture(fixtureDir, 'SPRINT-99', [
    ...
    assert.strictEqual(gateCheckCalls['STORY-099-02'], undefined);
...
describe('Scenario 19 (CR-038): Step 0 — all items error, Step 0 never calls exitFn(1)', () => {
  test('exit code is 0 when all gate checks error (Step 5 still runs and passes)', () => {
    ...
    const out = cap.getOut();
    assert.ok(String(out).includes('Step 0: refreshed 0 items, 2 errors.'));
    // Error bullet lines present in stdout
    assert.ok(String(out).includes('CR-099'));
    assert.ok(String(out).includes('EPIC-099'));
