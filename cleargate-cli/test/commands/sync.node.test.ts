...
describe('Scenario: sync pulls before pushing (R2)', () => {
  ...
  test('pullBeforePush: all cleargate_pull_item calls happen before push_item calls', async () => {
    ...
    assert.ok(String(callOrder).includes('cleargate_list_remote_updates'));
    assert.ok(String(callOrder).includes('cleargate_pull_item'));
    ...
    if (pushIndices.length === 0) {
      assert.strictEqual(pullItemIndices.length, 2);
    }

    const listUpdateIdx = callOrder.indexOf('cleargate_list_remote_updates');
    for (const pullIdx of pullItemIndices) {
      assert.ok(listUpdateIdx < pullIdx);
    }
  });
});
...
describe('Scenario: sync --dry-run mutates nothing', () => {
  ...
  test('dryRunZeroWrites: no fs changes, no sync-log entries', async () => {
    ...
    assert.ok(String(combined).includes('Would pull:'));
...
describe('Scenario: conflict halts sync and writes .conflicts.json', () => {
  ...
  test('conflictWritesJson: refused item in .conflicts.json, clean pull still applies', async () => {
    ...
    assert.ok(conflictsData.unresolved.length >= 1);
    ...
    assert.ok(String(cleanContent).includes('in-progress'));
  });
});
...
describe('Scenario: last_synced_status dataflow', () => {
  ...
  test('statusStampedOnApply: last_synced_status set to remote status after pull', async () => {
    ...
    assert.ok(String(content).includes('last_synced_status'));
    assert.ok(String(content).includes('in-progress'));
    assert.ok(String(content).includes('last_synced_body_sha'));
  });
});
...
describe('Scenario: last_synced fields stamped on push apply', () => {
  ...
  test('syncFieldsStampedOnPush: last_synced_status and last_synced_body_sha written after push', async () => {
    ...
    assert.strictEqual(resultFm['last_synced_status'], 'in-progress');
    // last_synced_body_sha must equal hashNormalized(body at push time)
    assert.strictEqual(resultFm['last_synced_body_sha'], hashNormalized(localBody));
  });
});
...
describe('Scenario: no-op-stub pre-flight refusal', () => {
  ...
  test('rejectsNoopStub: exits 2 when adapter is no-adapter-configured', async () => {
    const mcp = makeMcpClient({
    ...
    assert.ok(String(combined).includes('no PM adapter configured'));
    // Ensure no token appears in stderr
    assert.doesNotMatch(String(combined), /eyJ[A-Za-z0-9._-]+/);
  });

  test('no tokens appear in sync-log entries', async () => {
    ...
    if (fs.existsSync(sprintRunsDir)) {
      ...
      for (const dir of dirs) {
        ...
        if (fs.existsSync(logPath)) {
          ...
          assert.doesNotMatch(String(content), /eyJhbGciOiJIUzI1NiJ9\.eyJzdWIiOiJ0ZXN0In0\.fakeSig/);
        }
      }
    }
  });

  test('exits 2 with clear message when adapterInfo returns configured=false', async () => {
    const mcp: McpClient = {
...
describe('Scenario: missing CLEARGATE_MCP_TOKEN', () => {
  ...
  test('exits 2 when no MCP URL or token is configured', async () => {
    let exitCode: number | undefined;
    const stderrLines: string[] = [];
...
describe('Scenario: content-content conflict invokes merge helper', () => {
  ...
  test('merge prompt invoked for content-content conflict; kept choice preserves local body', async () => {
    ...
    const combined = stdoutLines.join('');
    assert.ok(String(combined).includes('[k]eep mine'));
    ...
    const content = fs.readFileSync(filePath, 'utf8');
    assert.ok(String(content).includes('Local modified body'));
    assert.ok(String(content).includes('in-progress'));  // status updated from remote
  });
});
...
describe('Scenario: sync --dry-run stdout format', () => {
  ...
  test('dry-run stdout prints Would pull: N, push: M, conflicts: K', async () => {
    const mcp = makeMcpClient({
      remoteRefs: [],
    });
    ...
    const combined = stdoutLines.join('');
    assert.match(String(combined), /Would pull: \d+, push: \d+, intake: \d+, conflicts: \d+/);
  });

  test('reads vitest successfully: placeholder test', async () => {
    // Smoke test to ensure module imports cleanly
    const { syncHandler: sh } = await import('../../src/commands/sync.js');
    assert.strictEqual(typeof sh, 'function');
  });
});
...
describe('Scenario: intake surfaces in sync stdout', () => {
  ...
  test('syncPrintsIntakeSummaryToStdout: live-path stdout contains emoji-prefixed intake line', async () => {
    ...
    assert.ok(String(combined).includes('📥 2 new stakeholder proposals pulled:'));
    // Must contain both proposals in the inline list
    assert.ok(String(combined).includes("LIN-101 'Enable SSO Login'"));
    assert.ok(String(combined).includes("LIN-102 'Dark Mode Support'"));
    // Must contain the trailing detail line
    assert.ok(String(combined).includes('— review at .cleargate/delivery/pending-sync/'));
  });
});
