...
describe('Sprint init decomposition gate: fixture sprint with epics: ["EPIC-X"] but no stories', () => {
  test('Scenario 12 (integration): sprint plan referencing EPIC-500 with no file → gate blocks', () => {
    ...
    assert.strictEqual(result.missing[0]?.id, 'EPIC-500');
    assert.strictEqual(result.missing[0]?.reason, 'file-missing');
    // Caller (sprintInitHandler) would exit 1 here
  });

  test('Scenario 10 (integration): EPIC-501 file exists but no child stories → gate blocks', () => {
  ...
  test('Scenario 9 (integration): EPIC-502 fully decomposed → gate passes', () => {
    const { root, pendingDir, sprintPlanPath } = makeDeliveryRoot({
      epics: ['EPIC-502'],
      proposals: [],
    });
...
describe('Scenario 13 (integration): --allow-drift does NOT waive decomposition gate', () => {
  test('sprintInitHandler logic: missing.length > 0 always blocks regardless of allowDrift', () => {
    const { root, sprintPlanPath } = makeDeliveryRoot({
      epics: ['EPIC-600'],
      proposals: [],
    });
    ...
    assert.strictEqual(shouldBlock, true); // gate blocks regardless of allowDrift
  });

  test('stderr message format for decomp waiver attempt', () => {
    ...
    assert.ok(String(expectedMessage).includes('decomposition gate cannot be waived'));
    assert.ok(String(expectedMessage).includes('push start_date'));
  });
});
...
describe('Sprint init: partial decomposition state', () => {
  test('one epic clean + one missing → gate still blocks (any missing = block)', () => {
    const { root, pendingDir, sprintPlanPath } = makeDeliveryRoot({
    ...
    assert.strictEqual((result.missing).length, 1);
    assert.strictEqual(result.missing[0]?.id, 'EPIC-702');
    assert.strictEqual(result.clean, 1);
    // Caller: if (result.missing.length > 0) exit(1)
    assert.strictEqual(result.missing.length > 0, true);
  });

  test('no epics or proposals → gate passes with missing=[], clean=0', () => {
    const { root, sprintPlanPath } = makeDeliveryRoot({
      epics: [],
      proposals: [],
    });
