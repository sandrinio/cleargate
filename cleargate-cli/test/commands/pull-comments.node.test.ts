...
describe('Scenario: --comments overrides active criteria', () => {
  ...
  test('pull <ID> --comments pulls comments and renders wiki section regardless of active criteria', async () => {
    ...
    assert.strictEqual(commentPullCalled, true);
    assert.strictEqual(commentPullCalledForId, remoteId);

    // Wiki page now has the comments section
    const wikiContent = await fsPromises.readFile(wikiPage, 'utf8');
    assert.ok(String(wikiContent).includes('<!-- cleargate:comments:start -->'));
    assert.ok(String(wikiContent).includes('<!-- cleargate:comments:end -->'));
    assert.ok(String(wikiContent).includes('## Remote comments'));
    assert.ok(String(wikiContent).includes('_Read-only snapshot. Comments live in the PM tool — reply there, not here._'));
    assert.ok(String(wikiContent).includes('> This looks good!'));
    ...
    assert.strictEqual((cachedComments).length, 1);
    assert.strictEqual(cachedComments[0].body, 'This looks good!');

    // Stdout reported comments
    const combinedOutput = stdoutLines.join('');
    assert.ok(String(combinedOutput).includes('comments fetched (1)'));
  });
});
