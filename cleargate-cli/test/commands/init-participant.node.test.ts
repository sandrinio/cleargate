...
describe('Scenario: init prompts and writes participant.json', () => {
  ...
  test('interactive happy path writes email with source=prompted', async () => {
    ...
    assert.strictEqual(fs.existsSync(participantPath), true);
    ...
    assert.strictEqual(content.email, 'ok@x.com');
...
describe('Scenario: init non-interactive accepts git default', () => {
  ...
  test('--yes flag uses git email with source=inferred and never invokes promptEmail', async () => {
    let promptCalled = false;
    ...
    assert.strictEqual(content.email, 'default@x.com');
    assert.strictEqual(content.source, 'inferred');
  });

  test('subsequent init run skips participant prompt when file exists', async () => {
    ...
    assert.strictEqual(content.email, 'existing@x.com');
  });
});
