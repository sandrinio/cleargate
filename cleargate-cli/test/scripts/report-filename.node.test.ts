...
describe('reportFilename — frozen API (CR-022 M0)', () => {
  ...
  test('scenario 1: write-side default returns new-name path (no fs probe)', () => {
    // reportFilename(dir, 'SPRINT-18') returns <dir>/SPRINT-18_REPORT.md
    const dir = makeTmpDir();
    const result = reportFilename(dir, 'SPRINT-18');
    assert.strictEqual(result, path.join(dir, 'SPRINT-18_REPORT.md'));
  });

  test('scenario 2: forRead=true with new-name file present returns new-name path', () => {
  ...
  test('scenario 3: forRead=true with only legacy REPORT.md present returns legacy path', () => {
    ...
    assert.strictEqual(result, path.join(dir, 'REPORT.md'));
  });

  test('scenario 4: forRead=true with neither file present returns new-name path', () => {
    ...
    const dir = makeTmpDir();
    const result = reportFilename(dir, 'SPRINT-15', { forRead: true });
    assert.strictEqual(result, path.join(dir, 'SPRINT-15_REPORT.md'));
  });

  test('scenario 5: no opts with only legacy REPORT.md present still returns new-name path', () => {
  ...
  test('scenario 6: non-numeric sprint id returns plain REPORT.md', () => {
  ...
  test('scenario 7: non-numeric sprint id with forRead=true still returns plain REPORT.md', () => {
  ...
  test('scenario 8: forRead=true with both files present — new-name wins', () => {
    ...
    assert.strictEqual(result, path.join(dir, 'SPRINT-19_REPORT.md'));
  });
