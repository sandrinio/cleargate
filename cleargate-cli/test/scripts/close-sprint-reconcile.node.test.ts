...
describe('reconcileLifecycle lib integration: fixture sprint with drift', () => {
  test('Scenario: fixture sprint with Draft artifact → drift returned', () => {
    ...
    assert.strictEqual(result.drift[0]?.actual_status, 'Draft');
    assert.ok(String(result.drift[0]?.commit_shas).includes('deadbeef'));
    assert.strictEqual(result.drift[0]?.in_archive, false);
  });

  test('Scenario: same fixture with Completed artifact in archive → clean', () => {
    const root = makeTmpDir();
    const pendingDir = path.join(root, 'pending-sync');
    const archiveDir = path.join(root, 'archive');
    fs.mkdirSync(pendingDir, { recursive: true });
...
describe('close_sprint.mjs: clean sprint (no lifecycle drift)', () => {
  test('exits 0 when all stories are Done and Step 2.6 finds no drift', () => {
    ...
    assert.ok(String([0, null]).includes(status));
    // Should not contain "non-terminal stories" error
    assert.ok(!String(stderr).includes('non-terminal stories'));
  });

  test('exits 1 when stories are not terminal (pre-existing Step 2 block)', () => {
    const sprintDir = makeTempSprintDir({
    ...
    assert.ok(String(stderr).includes('non-terminal'));
  });
});
...
describe('Lifecycle punch list format', () => {
  test('DriftItem contains all required fields for punch list emission', () => {
    const root = makeTmpDir();
    const pendingDir = path.join(root, 'pending-sync');
    const archiveDir = path.join(root, 'archive');
    fs.mkdirSync(pendingDir, { recursive: true });
    ...
    assert.strictEqual(item.id, 'CR-555');
    assert.strictEqual(item.type, 'CR');
    assert.strictEqual(item.actual_status, 'Draft');
    assert.notStrictEqual(item.expected_status, undefined);
    assert.ok(String(item.file_path).includes('pending-sync'));
    assert.ok(String(item.commit_shas).includes('sha12345'));
    assert.strictEqual(item.in_archive, false);
    assert.strictEqual(item.carry_over, false);
  });
});
