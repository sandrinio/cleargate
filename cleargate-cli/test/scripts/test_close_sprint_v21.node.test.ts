...
describe('Scenario 1: reporter.md documents v2.1 contract', () => {
  test('scaffold reporter.md contains "Fast-Track Ratio"', () => {
    const content = fs.readFileSync(REPORTER_MD_SCAFFOLD, 'utf8');
    assert.match(String(content), /Fast-Track Ratio/);
  });

  test('scaffold reporter.md contains "Lane Audit"', () => {
    const content = fs.readFileSync(REPORTER_MD_SCAFFOLD, 'utf8');
    assert.match(String(content), /Lane Audit/);
  });

  test('scaffold reporter.md contains a "Sprint Report v2.1" section heading', () => {
    const content = fs.readFileSync(REPORTER_MD_SCAFFOLD, 'utf8');
    assert.match(String(content), /Sprint Report v2\.1/);
  });

  test('scaffold reporter.md enumerates all six §3 metric row names', () => {
    const content = fs.readFileSync(REPORTER_MD_SCAFFOLD, 'utf8');
    assert.match(String(content), /Fast-Track Ratio/);
    assert.match(String(content), /Fast-Track Demotion Rate/);
    assert.match(String(content), /Hotfix Count/);
    assert.match(String(content), /Hotfix-to-Story Ratio/);
    assert.match(String(content), /Hotfix Cap Breaches/);
    assert.match(String(content), /LD events/);
  });

  test('scaffold reporter.md enumerates Lane Audit + Hotfix Audit + Hotfix Trend', () => {
    const content = fs.readFileSync(REPORTER_MD_SCAFFOLD, 'utf8');
    assert.match(String(content), /Lane Audit/);
    assert.match(String(content), /Hotfix Audit/);
    assert.match(String(content), /Hotfix Trend/);
  });
});
...
describe('Scenario 2: close_sprint accepts a v1 report unchanged (legacy-pass)', () => {
  ...
  test('exits 0 for a v1 state.json (no v2 validation triggered)', () => {
    const result = runCloseSprint(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('no error about missing Lane Audit or Hotfix Audit is emitted', () => {
    const result = runCloseSprint(sprintDir);
    assert.doesNotMatch(String(result.stderr), /Lane Audit/);
    assert.doesNotMatch(String(result.stderr), /Hotfix Audit/);
    assert.doesNotMatch(String(result.stderr), /§3 missing rows/);
    assert.doesNotMatch(String(result.stderr), /§5 missing/);
  });

  test('state.json sprint_status is flipped to Completed', () => {
    runCloseSprint(sprintDir);
    const state = JSON.parse(fs.readFileSync(path.join(sprintDir, 'state.json'), 'utf8'));
    assert.strictEqual(state.sprint_status, 'Completed');
  });
});
...
describe('Scenario 3: close_sprint accepts a v2 report with no fast-lane stories', () => {
  ...
  test('exits 0 (activation gate not met — no lane=fast story)', () => {
    const result = runCloseSprint(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('no v2.1 validation error is emitted', () => {
    const result = runCloseSprint(sprintDir);
    assert.doesNotMatch(String(result.stderr), /§3 missing rows/);
    assert.doesNotMatch(String(result.stderr), /§5 missing/);
    assert.doesNotMatch(String(result.stderr), /Lane Audit/);
...
describe('Scenario 4: close_sprint rejects a v2 report missing required sections', () => {
  ...
  test('exits non-zero (validation fails)', () => {
    const result = runCloseSprint(sprintDir);
    assert.notStrictEqual(result.status, 0);
    assert.strictEqual(result.status, 1);
  });

  test('error names the missing "Lane Audit" section verbatim', () => {
    const result = runCloseSprint(sprintDir);
    assert.match(String(result.stderr), /Lane Audit/);
  });

  test('stderr contains §5 missing message', () => {
    const result = runCloseSprint(sprintDir);
    assert.match(String(result.stderr), /§5 missing/);
  });

  test('state.json sprint_status stays Active (not flipped)', () => {
    runCloseSprint(sprintDir);
    const state = JSON.parse(fs.readFileSync(path.join(sprintDir, 'state.json'), 'utf8'));
    assert.strictEqual(state.sprint_status, 'Active');
  });
});
...
describe('Scenario 5: close_sprint rejects non-conformant sprint-run path', () => {
  ...
  test('exits non-zero with naming convention error', () => {
    const result = runCloseSprint(sprintDir);
    assert.strictEqual(result.status, 1);
  });

  test('stderr explains the ^SPRINT-\\d{2,3}$ convention', () => {
    const result = runCloseSprint(sprintDir);
    // The error message includes the regex pattern
    assert.match(String(result.stderr), /SPRINT-\\d\{2,3\}/);
  });

  test('stderr mentions the offending path name', () => {
    const result = runCloseSprint(sprintDir);
    assert.match(String(result.stderr), /S-99/);
  });
});
...
describe('Scenario 6: Reporter writes Lane Audit + Hotfix Audit on a sprint with a fast-lane story', () => {
  ...
  test('exits 0 when REPORT.md has all v2.1 sections', () => {
    const result = runCloseSprint(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('v2.1 validation passes (Step 2.5 pass logged to stdout)', () => {
    const result = runCloseSprint(sprintDir);
    assert.match(String(result.stdout), /Step 2\.5 passed/);
  });

  test('REPORT.md contains all six §3 metric rows', () => {
    ...
    assert.match(String(report), /Fast-Track Ratio/);
    assert.match(String(report), /Fast-Track Demotion Rate/);
    assert.match(String(report), /Hotfix Count/);
    assert.match(String(report), /Hotfix-to-Story Ratio/);
    assert.match(String(report), /Hotfix Cap Breaches/);
    assert.match(String(report), /LD events/);
  });

  test('REPORT.md contains Lane Audit section', () => {
    const report = fs.readFileSync(path.join(sprintDir, 'REPORT.md'), 'utf8');
    assert.match(String(report), /Lane Audit/);
  });

  test('REPORT.md contains Hotfix Audit section', () => {
    const report = fs.readFileSync(path.join(sprintDir, 'REPORT.md'), 'utf8');
    assert.match(String(report), /Hotfix Audit/);
  });

  test('REPORT.md contains Hotfix Trend section', () => {
    const report = fs.readFileSync(path.join(sprintDir, 'REPORT.md'), 'utf8');
    assert.match(String(report), /Hotfix Trend/);
  });
...
describe('Scenario 7: SPRINT-18+ sprints write SPRINT-<#>_REPORT.md (not REPORT.md)', () => {
  ...
  test('writes SPRINT-18_REPORT.md when piped via stdin', () => {
  ...
  test('does NOT create plain REPORT.md when using new naming', () => {
    const reportBody = '# Sprint Report — SPRINT-18\n\n## §1 What Was Delivered\n- Test\n';
    const stateFile = path.join(sprintDir, 'state.json');
...
describe('Scenario 8: SPRINT-01..17 archived REPORT.md keeps old name (backwards-compat)', () => {
  ...
  test('exits 0 when closed with --assume-ack', () => {
    const result = runCloseSprint(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('legacy REPORT.md is NOT renamed or deleted', () => {
    runCloseSprint(sprintDir);
    expect(fs.existsSync(path.join(sprintDir, 'REPORT.md'))).toBe(true);
  });

  test('SPRINT-15_REPORT.md is NOT created', () => {
    runCloseSprint(sprintDir);
...
describe('Scenario 9: Step 3.5 invokes prep_reporter_context.mjs and prints step message', () => {
  ...
  test('stdout or stderr contains "Step 3.5" message', () => {
    const result = runCloseSprint(sprintDir);
    // Step 3.5 is always attempted — check combined output mentions it
    assert.match(String(result.stdout + result.stderr), /Step 3\.5/);
  });

  test('pipeline exits 0 when Step 3.5 finishes (pass or warn)', () => {
    const result = runCloseSprint(sprintDir);
    assert.strictEqual(result.status, 0);
  });
});
...
describe('Scenario 10: Step 3.5 warns and continues if prep_reporter_context.mjs fails', () => {
  ...
  test('stderr contains "Step 3.5 warning" when prep_reporter_context.mjs fails', () => {
    const result = runWithBundleCheck(sprintDir);
    assert.match(String(result.stderr), /Step 3\.5 warning/);
  });

  test('pipeline exits 0 despite Step 3.5 failure (non-fatal)', () => {
    const result = runWithBundleCheck(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('sprint_status is flipped to Completed even when Step 3.5 fails', () => {
...
describe('Scenario 11: Step 7 skipped when cleargate-cli/dist/cli.js does not exist', () => {
  ...
  test('stdout contains "Step 7 skipped: CLI binary not found" when dist/cli.js absent', () => {
    ...
    const result = runCloseSprint(sprintDir);
    assert.match(String(result.stdout), /Step 7 skipped: CLI binary not found/);
  });

  test('exit code is 0 whether Step 7 skips or warns', () => {
    const result = runCloseSprint(sprintDir);
    assert.strictEqual(result.status, 0);
  });
});
...
describe('Scenario 12: Step 7 warns and continues when sync work-items exits non-zero', () => {
  ...
  test('sprint_status stays Completed even if Step 7 warns (Step 5 already flipped it)', () => {
  ...
  test('stdout contains "Step 7" (either passed or skipped or warned)', () => {
    const result = runCloseSprint(sprintDir);
    assert.match(String(result.stdout + result.stderr), /Step 7/);
  });
});
...
describe('Scenario 13: Step 5 wait-for-ack prompt shows SPRINT-<#>_REPORT.md filename', () => {
  ...
  test('Step 4 announcement mentions SPRINT-18_REPORT.md (not plain REPORT.md)', () => {
    const stateFile = path.join(sprintDir, 'state.json');
    ...
    assert.match(String(result.stdout), /SPRINT-18_REPORT\.md/);
  });
});
...
describe('Scenario 14: Sprint ID with no numeric portion (SPRINT-TEST) uses plain REPORT.md', () => {
  ...
  test('writes plain REPORT.md (not SPRINT-TEST_REPORT.md) for non-numeric sprint IDs', () => {
...
describe('Scenario 15: Step 2.7 skipped when CLEARGATE_SKIP_WORKTREE_CHECK=1 (test seam)', () => {
  ...
  test('stdout contains "Step 2.7 skipped: CLEARGATE_SKIP_WORKTREE_CHECK=1 set (test seam)"', () => {
    const result = runWithSkipWorktreeCheck(sprintDir);
    assert.match(String(result.stdout), /Step 2\.7 skipped: CLEARGATE_SKIP_WORKTREE_CHECK=1 set \(test seam\)/);
  });

  test('exit code is 0 when Step 2.7 skips via test seam', () => {
    const result = runWithSkipWorktreeCheck(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('pipeline proceeds past Step 2.7 (Step 3 message also appears)', () => {
    const result = runWithSkipWorktreeCheck(sprintDir);
    assert.match(String(result.stdout), /Step 3/);
  });
...
describe('Scenario 16: Step 2.7 blocks under v2 when leftover worktree present (CLEARGATE_FORCE_WORKTREE_PATHS env seam)', () => {
  ...
  test('exits 1 when a STORY-prefixed leftover worktree is injected under v2', () => {
    const result = runWithFakeWorktree(sprintDir, '.worktrees/STORY-019-01');
    assert.strictEqual(result.status, 1);
  });

  test('stderr contains "Step 2.7 failed: leftover worktree at .worktrees/STORY-019-01"', () => {
    const result = runWithFakeWorktree(sprintDir, '.worktrees/STORY-019-01');
    assert.match(String(result.stderr), /Step 2\.7 failed: leftover worktree at \.worktrees\/STORY-019-01/);
  });

  test('stderr contains git worktree remove hint', () => {
    const result = runWithFakeWorktree(sprintDir, '.worktrees/STORY-019-01');
    assert.match(String(result.stderr), /git worktree remove/);
  });

  test('state.json sprint_status stays Active (not flipped to Completed)', () => {
...
describe('Scenario 17: Step 2.7 advisory under v1 — warn + continue (CLEARGATE_FORCE_WORKTREE_PATHS env seam)', () => {
  ...
  test('exit code is 0 despite leftover worktree (advisory in v1)', () => {
    const result = runWithFakeWorktreeV1(sprintDir, '.worktrees/STORY-019-02');
    assert.strictEqual(result.status, 0);
  });

  test('stderr contains "Step 2.7 warning: leftover worktree at .worktrees/STORY-019-02 (advisory in v1)"', () => {
    const result = runWithFakeWorktreeV1(sprintDir, '.worktrees/STORY-019-02');
    assert.match(String(result.stderr), /Step 2\.7 warning: leftover worktree at \.worktrees\/STORY-019-02 \(advisory in v1\)/);
  });
  ...
  test('state.json sprint_status is flipped to Completed (pipeline continues)', () => {
...
describe('Scenario 18: Step 2.8 skipped via CLEARGATE_SKIP_MERGE_CHECK=1 (test seam)', () => {
  ...
  test('stdout contains "Step 2.8 skipped: CLEARGATE_SKIP_MERGE_CHECK=1 set (test seam)"', () => {
    const result = runWithSkipMergeCheck(sprintDir);
    assert.match(String(result.stdout), /Step 2\.8 skipped: CLEARGATE_SKIP_MERGE_CHECK=1 set \(test seam\)/);
  });

  test('exit code is 0 when Step 2.8 skips via test seam', () => {
    const result = runWithSkipMergeCheck(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('pipeline proceeds past Step 2.8 (Step 3 message also appears)', () => {
    const result = runWithSkipMergeCheck(sprintDir);
    assert.match(String(result.stdout), /Step 3/);
  });
...
describe('Scenario 19: Step 2.8 blocks under v2 when sprint branch unmerged (CLEARGATE_FORCE_MERGE_STATUS=unmerged seam)', () => {
  ...
  test('exits 1 when sprint branch is unmerged under v2', () => {
    const result = runWithForcedUnmergedV2(sprintDir);
    assert.strictEqual(result.status, 1);
  });

  test('stderr contains "Step 2.8 failed: sprint/S-19 not merged to main"', () => {
    const result = runWithForcedUnmergedV2(sprintDir);
    assert.match(String(result.stderr), /Step 2\.8 failed: sprint\/S-19 not merged to main/);
  });

  test('stderr contains "Resolve: merge sprint/S-19" hint', () => {
    const result = runWithForcedUnmergedV2(sprintDir);
    assert.match(String(result.stderr), /Resolve: merge sprint\/S-19/);
  });
...
describe('Scenario 20: Step 2.8 advisory under v1 — warn + continue (CLEARGATE_FORCE_MERGE_STATUS=unmerged seam)', () => {
  ...
  test('exit code is 0 despite unmerged sprint branch (advisory in v1)', () => {
    const result = runWithForcedUnmergedV1(sprintDir);
    assert.strictEqual(result.status, 0);
  });

  test('stderr contains "Step 2.8 warning: sprint/S-19 not merged to main (advisory in v1)"', () => {
    const result = runWithForcedUnmergedV1(sprintDir);
    assert.match(String(result.stderr), /Step 2\.8 warning: sprint\/S-19 not merged to main \(advisory in v1\)/);
  });
...
describe('Scenario 21: Step 6.5 sprint_trends stub writes Trends section', () => {
  ...
  test('improvement-suggestions.md contains "## Trends" section after pipeline runs', () => {
    ...
    assert.match(String(content), /## Trends/);
    assert.match(String(content), /full analysis deferred to CR-027/);
  });

  test('stdout contains "Step 6.5:" message', () => {
    const sprintRunsDir = path.dirname(sprintDir);
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-TEST', '--assume-ack'],
    ...
    assert.match(String(result.stdout), /Step 6\.5/);
  });
});
...
describe('Scenario 22: Step 6.5 sprint_trends warns + continues if stub fails', () => {
  ...
  test('exit code is still 0 when sprint_trends.mjs is missing (non-fatal)', () => {
  ...
  test('stderr contains "Step 6.5 warning" when sprint_trends.mjs is missing', () => {
    const fakeCloseSprint = path.join(fakeScriptsDir, 'close_sprint.mjs');
    const result = spawnSync(
      '/usr/bin/env',
      ['node', fakeCloseSprint, 'SPRINT-TEST', '--assume-ack'],
    ...
    assert.match(String(result.stderr), /Step 6\.5 warning/);
  });
});
...
describe('Scenario 23: Step 6.6 skill candidates section emitted (placeholder when empty)', () => {
  ...
  test('improvement-suggestions.md contains "## Skill Creation Candidates" section', () => {
    const result = runWithSkipAll();
    assert.strictEqual(result.status, 0);
    const suggestionsFile = path.join(sprintDir, 'improvement-suggestions.md');
    expect(fs.existsSync(suggestionsFile)).toBe(true);
    const content = fs.readFileSync(suggestionsFile, 'utf8');
    assert.match(String(content), /## Skill Creation Candidates/);
  });

  test('section contains placeholder "_No candidates detected this sprint._" when no ledger or matching flashcards', () => {
    const result = runWithSkipAll();
    assert.strictEqual(result.status, 0);
    const content = fs.readFileSync(path.join(sprintDir, 'improvement-suggestions.md'), 'utf8');
    assert.match(String(content), /_No candidates detected this sprint\./);
  });

  test('stdout contains "Step 6.6:" message', () => {
    const result = runWithSkipAll();
    assert.match(String(result.stdout), /Step 6\.6/);
  });
});
...
describe('Scenario 24: Step 6.6 surfaces CAND entry when ledger has ≥3× repeated tuple', () => {
  ...
  test('improvement-suggestions.md contains CAND-SPRINT-TEST-S entry', () => {
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-TEST', '--assume-ack'],
      {
    ...
    assert.strictEqual(result.status, 0);
    const content = fs.readFileSync(path.join(sprintDir, 'improvement-suggestions.md'), 'utf8');
    assert.match(String(content), /CAND-SPRINT-TEST-S\d+/);
    assert.match(String(content), /STORY-TEST-01/);
  });
});
...
describe('Scenario 25: Step 6.7 FLASHCARD cleanup section emitted (placeholder when empty)', () => {
  ...
  test('improvement-suggestions.md contains "## FLASHCARD Cleanup Candidates" section', () => {
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-TEST', '--assume-ack'],
      {
    ...
    expect(fs.existsSync(suggestionsFile)).toBe(true);
    const content = fs.readFileSync(suggestionsFile, 'utf8');
    assert.match(String(content), /## FLASHCARD Cleanup Candidates/);
  });

  test('section contains placeholder "_No candidates detected this sprint._" when FLASHCARD.md is empty', () => {
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-TEST', '--assume-ack'],
      {
  ...
  test('stdout contains "Step 6.7:" message', () => {
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-TEST', '--assume-ack'],
      {
    ...
    assert.match(String(result.stdout), /Step 6\.7/);
  });
});
...
describe('Scenario 26: Step 6.7 surfaces stale candidate when keyword has zero grep hits across last 3 sprint dirs', () => {
  ...
  test('improvement-suggestions.md contains CAND-SPRINT-19-F entry for stale flashcard', () => {
    const result = spawnSync(
      '/usr/bin/env',
    ...
    expect(fs.existsSync(suggestionsFile)).toBe(true);
    const content = fs.readFileSync(suggestionsFile, 'utf8');
    assert.match(String(content), /CAND-SPRINT-19-F\d+/);
    assert.match(String(content), /stale/);
  });
});
...
describe('Scenario 27: Step 8 prints 6-item handoff list (CR-022 §3 M4)', () => {
  ...
  test('stdout contains all 6 handoff list items', () => {
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-19', '--assume-ack'],
      {
    ...
    assert.match(String(result.stdout), /1\. Review .+REPORT\.md/);
    // Item 2: Review improvement-suggestions.md
    assert.match(String(result.stdout), /2\. Review improvement-suggestions\.md/);
    // Item 3: Skill Candidates
    assert.match(String(result.stdout), /3\. Approve or reject Skill Candidates/);
    // Item 4: FLASHCARD cleanup
    assert.match(String(result.stdout), /4\. Approve or reject FLASHCARD cleanup entries/);
    // Item 5: MCP sync
    assert.match(String(result.stdout), /5\. Push approved status changes to MCP/);
    // Item 6: Initialize next sprint
    assert.match(String(result.stdout), /6\. Initialize next sprint/);
  });

  test('Step 8 header line contains sprint ID "SPRINT-19 closed"', () => {
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-19', '--assume-ack'],
      {
    ...
    assert.strictEqual(result.status, 0);
    assert.match(String(result.stdout), /SPRINT-19 closed\. Next steps:/);
  });
});
...
describe('Scenario 28: Step 8 mentions incremented next-sprint ID (CR-022 §3 M4)', () => {
  ...
  test('item 6 contains SPRINT-20 as next sprint ID', () => {
    const result = spawnSync(
      '/usr/bin/env',
      ['node', CLOSE_SPRINT_SCRIPT, 'SPRINT-19', '--assume-ack'],
      {
    ...
    assert.strictEqual(result.status, 0);
    assert.match(String(result.stdout), /cleargate sprint init SPRINT-20/);
  });
});
