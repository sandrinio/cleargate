...
describe('Protocol §14 completeness and scaffold-mirror', () => {
  test('Scenario 9: all 9 §14 subsections present in dogfood protocol file', () => {
    const content = fs.readFileSync(DOGFOOD_PROTOCOL, 'utf8');
    for (const heading of EXPECTED_SUBSECTIONS) {
      assert.ok(String(content, `Missing in dogfood: ${heading}`).includes(heading));
    }
    // Also assert top-level § 14 section header
    assert.ok(String(content).includes('## 14. Multi-Participant Sync'));
  });

  test('Scenario 9b: all 9 §14 subsections present in scaffold mirror protocol file', () => {
    const content = fs.readFileSync(SCAFFOLD_PROTOCOL, 'utf8');
    for (const heading of EXPECTED_SUBSECTIONS) {
      assert.ok(String(content, `Missing in scaffold: ${heading}`).includes(heading));
    }
    assert.ok(String(content).includes('## 14. Multi-Participant Sync'));
  });

  test('Scenario 10: diff between dogfood and scaffold protocol files is empty (byte-identical)', () => {
