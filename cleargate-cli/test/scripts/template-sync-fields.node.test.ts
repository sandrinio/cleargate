...
describe('Scenario: Templates carry new sync-attribution fields (string presence)', () => {
  for (const name of ALL_TEMPLATES) {
    test(`live template ${name} contains all 6 new sync fields`, () => {
      const content = fs.readFileSync(path.join(LIVE_TEMPLATES_DIR, name), 'utf8');
      for (const field of NEW_FIELDS) {
        assert.ok(String(content, `${name} should contain field "${field}"`).includes(`${field}:`));
      }
    });
  }
});
...
describe('Scenario: Templates parseable via parseFrontmatter contain new fields in FM', () => {
  for (const name of PARSEABLE_TEMPLATES) {
    test(`live template ${name} — parseFrontmatter returns all 6 new keys`, () => {
      const content = fs.readFileSync(path.join(LIVE_TEMPLATES_DIR, name), 'utf8');
      const yamlBlock = extractYamlBlock(content);
      assert.notStrictEqual(yamlBlock, `${name} must have a ---...--- block`, null);

      const { fm } = parseFrontmatter(yamlBlock!);

      for (const field of NEW_FIELDS) {
        assert.ok(field in (fm, `${name} frontmatter should have key "${field}"`));
      }
    });

    test(`live template ${name} — frontmatter round-trips losslessly`, () => {
      const content = fs.readFileSync(path.join(LIVE_TEMPLATES_DIR, name), 'utf8');
      const yamlBlock = extractYamlBlock(content);
      assert.notStrictEqual(yamlBlock, null);
      ...
      assert.deepStrictEqual(roundTripped, originalFm);
    });
  }
});
...
describe('Scenario: Pre-existing frontmatter keys preserved', () => {
  for (const name of PARSEABLE_TEMPLATES) {
    test(`live template ${name} still has draft_tokens and cached_gate_result`, () => {
      const content = fs.readFileSync(path.join(LIVE_TEMPLATES_DIR, name), 'utf8');
