...
describe('Scenario: CLAUDE.md literal-rule paragraph (CR-032)', () => {
  test('live CLAUDE.md contains "Ambiguity Gate criteria are evaluated literally"', () => {
    const content = fs.readFileSync(LIVE_CLAUDE_MD, 'utf8');
    assert.ok(String(content).includes(LITERAL_RULE_PHRASE));
  });

  test('canonical cleargate-planning/CLAUDE.md contains "Ambiguity Gate criteria are evaluated literally"', () => {
...
describe('Scenario: 5 live templates carry the literal-criterion preamble (CR-032)', () => {
  for (const name of FOOTER_TEMPLATES) {
    test(`live template ${name} contains the literal-criterion preamble`, () => {
      const filePath = path.join(LIVE_TEMPLATES_DIR, name);
      const content = fs.readFileSync(filePath, 'utf8');
      assert.ok(String(content).includes(TEMPLATE_PREAMBLE_PHRASE));
    });
  }
});
...
describe('Scenario: 5 canonical templates carry the literal-criterion preamble (CR-032)', () => {
  for (const name of FOOTER_TEMPLATES) {
    test(`canonical template ${name} contains the literal-criterion preamble`, () => {
...
describe('Scenario: CLAUDE.md mirror parity for the new literal-rule paragraph (CR-032)', () => {
  ...
  test('live and canonical CLAUDE.md literal-rule paragraph text is byte-identical', () => {
    const liveBlock = extractLiteralRuleBlock(LIVE_CLAUDE_MD);
    const canonicalBlock = extractLiteralRuleBlock(CANONICAL_CLAUDE_MD);
    assert.notStrictEqual(liveBlock, ''); // guard: block was found
    assert.strictEqual(liveBlock, canonicalBlock);
  });
});
...
describe('Scenario: 5 template live/canonical pairs are byte-identical (CR-032)', () => {
  for (const name of FOOTER_TEMPLATES) {
    test(`template ${name}: live preamble region === canonical preamble region`, () => {
      ...
      assert.strictEqual(live, canonical);
    });
  }
});
