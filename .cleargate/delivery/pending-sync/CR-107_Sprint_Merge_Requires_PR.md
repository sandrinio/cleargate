...
status: Draft
approved: true
context_source: verified codebase grounding (SKILL.md:714 local sprint->main merge; cleargate-enforcement.md:110 walkthrough gate; git remote -v on all three repos) + recorded direct approval 2026-08-26
...
- **Forget that the sprint lands on `main` via a local merge.** `SKILL.md:714` runs `git merge sprint/S-NN --no-ff -m "Sprint S-NN: <goal>"` in the primary checkout. Nothing outside the local machine ever sees the sprint diff as a reviewable object.
...
- **Story→sprint merges are unchanged.** DevOps keeps merging locally, one worktree at a time (`SKILL.md:445`). This CR touches exactly one merge in the whole sprint.
...
- **Surface:** `.claude/skills/sprint-execution/SKILL.md:714` — `git merge sprint/S-NN --no-ff`; the local sprint→main merge this CR routes through a PR.
...
**Do NOT modify:** DevOps agent, story→sprint merge (`SKILL.md:445`, §C.7), worktree lifecycle.
