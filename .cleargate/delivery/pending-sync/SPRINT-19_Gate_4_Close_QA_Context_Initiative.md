...
- **Wave 1 — 4 parallel anchors over disjoint surfaces:** BUG-023 ‖ M1 (CR-022 M0 helper) ‖ M2 (CR-024 S1) ‖ M3 (CR-025 S1).
...
**Note on BUG-023.** Investigation-only — no production code change. The spike output is a follow-up CR-026 candidate scoped at ~100 LOC for SPRINT-20. BUG-023 ships in M0 as a parallel anchor with no downstream blockers; its DoD is met by the diagnosis already captured in §3 of the bug file (drafted 2026-05-01).
...
- **BUG-023 has no production code surface.** It's the spike investigation. Its DoD is met by the diagnosis already in the bug file. M0 dev dispatch is essentially a no-op confirmation pass (orchestrator may skip it if QA accepts the bug file as-is at sprint init time).
...
- **Soft flag (informational):** BUG-023 is investigation-only; CR-026 lands the fix in SPRINT-20. SPRINT-19's REPORT.md should explicitly flag CR-026 in §6 Tooling so the SPRINT-20 sprint-init pass picks it up.
...
| **Token-ledger Red carries forward into SPRINT-19.** BUG-023 diagnoses; CR-026 fixes in SPRINT-20. SPRINT-19's own ledger will continue to mis-attribute. | Accept the carry. SPRINT-19 REPORT.md §6 Tooling explicitly notes this + flags CR-026. SPRINT-20 sprint-init picks up CR-026 as the first anchor. |
...
- **Constraints:**
  ...
  - Token-ledger Red is OUT of scope for fixing in SPRINT-19. BUG-023 only diagnoses. Surface in §4 Execution Log if you observe attribution surprises, but do not implement.
...
- [x] All 4 anchor items decomposed and 🟢 (CR-022 + CR-024 + CR-025 + BUG-023).
