...
stamp_error: no ledger rows for work_item_id STORY-051-08
draft_tokens:
  input: null
  output: null
  cache_creation: null
  cache_read: null
  model: null
  last_stamp: 2026-07-17T18:52:13Z
  sessions: []
---
...
| Item | Value |
|---|---|
| Primary File (canonical) | `cleargate-planning/.cleargate/scripts/close_sprint.mjs` — add the `--assume-ack` CI-token guard after arg parse (~line 177) |
| Primary File (canonical) | `cleargate-planning/.cleargate/knowledge/cleargate-enforcement.md` — narrow §15 advisory scope (line 540); document guard in §12.3 (lines 455-457) |
| Related File (canonical) | `cleargate-planning/CLAUDE.md` — sprint-close guardrail wording (lines 36, 63) |
| Related File (root) | `CLAUDE.md` — sprint-close guardrail wording inside CLEARGATE block (lines 160, 185) + meta-repo guardrail (line 126) |
| New File (canonical) | `cleargate-planning/.cleargate/scripts/close_sprint.assume-ack-guard.node.test.ts` — node:test for the guard |
| Related File (canonical) | `cleargate-planning/.cleargate/scripts/close_sprint.deferred-verify.red.node.test.ts` — add `CLEARGATE_CI_ACK: '1'` to `baseEnv()` (~line 82) |
...
| Live mirror | `.cleargate/scripts/close_sprint.assume-ack-guard.node.test.ts` (hand-synced, NEW) |
...
| Payload mirror | `cleargate-cli/templates/cleargate-planning/.cleargate/scripts/close_sprint.assume-ack-guard.node.test.ts` (regenerated by prebuild, NEW) |
...
| New Files Needed | Yes — one test file (`close_sprint.assume-ack-guard.node.test.ts`) in canonical + live + payload |
...
| Grounding refs (read-only) | `cleargate-cli/src/util/gate-mode.ts:14`, `cleargate-cli/src/commands/sprint.ts:1681`, `cleargate-planning/.cleargate/scripts/init_sprint.mjs:140`, `cleargate-planning/.claude/hooks/pending-task-sentinel.sh:129` |
...
> Held at 🟡 (not 🟢) pending human ratification of the §1.4 NEW question: the implemented §15 scope enumerates BOTH `cleargate sprint preflight` and the `cleargate sprint init` story-file assertion, which exceeds the literal Q1 human-decision text ("softens only `cleargate sprint preflight`"). Grounding proves both sites already honor the lever, so no new honor site is added — but the recorded human decision must be updated to match before this goes Green.