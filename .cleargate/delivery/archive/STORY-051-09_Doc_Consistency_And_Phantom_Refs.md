...
draft_tokens:
  ...
  last_stamp: 2026-07-27T13:50:35Z
  sessions:
...
> **§3.1 self-amendment justification (file-surface contract §9).** `cleargate-enforcement.md` (canonical + live-root) and the protocol `:119` `§<N>` → `§13` fix were not in this story's original file list at draft time; both are added here per the M2.md milestone plan's 2026-07-27 orchestrator ruling on Open Decisions AD#2/AD#4. AD#2 assigns the enforcement §6.2 `:294-295` "Under v1/v2" bullets and the §12 heading/§12.1 "Gate 3.5 / Gate-3-class" naming to this story (both are literal four-gate-spine violations this story's R1 claims repo-wide coverage over, and 09 is already in the file editing adjacent regions). The `:20` §-index row 3rd-column retitle and the `:461` "Gate-3 breach" → "Gate-4 breach" gate-number reword are a separate grant — M2.md AD#5 — with the contemporaneous record at `.cleargate/sprint-runs/SPRINT-38/reports/STORY-051-08-arch.md` ("Wave-5 scope additions raised — both APPROVED by the orchestrator"); 08's `CLEARGATE_CI_ACK` sentence on `:461` is untouched — only the gate-number token moves to R14. AD#4 assigns the protocol §4 Gate-3 `:119` `§<N>` placeholder resolution (→ `cleargate-enforcement.md` §13) to this story as the same defect class as R5's phantom paths. AD#2 and AD#4 are recorded verbatim in `.cleargate/sprint-runs/SPRINT-38/plans/M2.md` § "Mid-Sprint Amendment (orchestrator, 2026-07-27)". Scope stayed within the four line regions AD#2 named (`:294-295`, `:448`, `:452`) plus the AD#5 pair (`:20`, `:461`) — the five carry-over `v1`/`v2` mentions and every `(source: protocol §NN)` annotation were left untouched per the plan's carry-over CR note.
