...
- **2026-06-01 — cleargate-cli stories execute in the cleargate-cli repo (cross-repo execution model).** `cleargate-cli/` is a separate gitignored repo (`sandrinio/cleargate-cli`), NOT present in outer-repo worktrees. Stories editing `cleargate-cli/src/**` (043-02 readiness-predicates, 043-04 work-item-type, 043-07 wiki-ingest, 043-09 cli.ts) therefore run via a worktree IN the cleargate-cli repo (`git -C cleargate-cli worktree add ... -b story/STORY-XXX main`); the QA-Red→TPV→Dev→QA-Verify→Architect loop operates there; DevOps merges story→cleargate-cli `main` **(--no-ff, LOCAL ONLY — no `git push`, no `npm publish`; release is the owner's separate action)**. Dispatch markers + ledger + reports stay in the OUTER repo's sprint-runs (agents given absolute paths). Sprint state.json (outer) tracks completion. Not a true-blocker (resolvable, local-only/reversible) — resolved per the post-start autonomy directive.
