# Improvement Suggestions — SPRINT-40


## Trends

Trends: 26 closed sprints visible — full analysis deferred to CR-027.

## Skill Creation Candidates

<!-- generated-by: suggest_improvements.mjs --skill-candidates -->

### CAND-SPRINT-40-S01: BUG-033 × architect
<!-- hash:7a87c1 -->

**Pattern detected:** BUG-033 × architect repeated ≥3× across ≥2 distinct sprints (SPRINT-39, SPRINT-40)
...
### CAND-SPRINT-40-F01: The Reporter's own SubagentStop CAN fire mid-dispatch (a rea
<!-- hash:1ffc7c -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-09-02 · #reporting #ledger #danger · The Reporter's own SubagentStop CAN fire mid-dispatch (a real reporter-attributed token-ledger row existed at report-write time, not the "TBD, row does not exist yet" reporter.md assumes) — check the ledger for a reporter row before defaulting Source 3 to a blanket TBD; report it as PARTIAL instead. [SPRINT-40 Reporter]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F02: A Goal Acceptance criterion of the form ">1 distinct agent_t
<!-- hash:a35558 -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-09-02 · #gate #ledger #danger · A Goal Acceptance criterion of the form ">1 distinct agent_type in the ledger" is SATISFIED BY THE FABRICATION IT EXISTS TO DETECT: SPRINT-40's own pre-fix ledger already carried 4 distinct values (93/101 `architect`, incl. `work_item_id: "M1"` — a plan filename — and 15 rows of cross-sprint `BUG-033`). A distinct-count assertion over an inference-based producer discriminates nothing; assert a SPECIFIC value on a SPECIFIC row. [SPRINT-40 BUG-069 post-flight]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F03: Post-BUG-069 an `unattributed` row needs a spawn that is mar
<!-- hash:8b4712 -->

**Category:** stale
**Reason:** stale: zero grep hits across last 2 sprint dir(s)
**Original entry:** `2026-09-02 · #hooks #ledger · Post-BUG-069 an `unattributed` row needs a spawn that is marker-less AND sentinel-less-or-"unknown" — but `pending-task-sentinel.sh` has no allow-list, so it attributes every spawn. A HEALTHY live ledger therefore shows ZERO `unattributed` rows: their absence is evidence of nothing. Verify a refusal path with its red test, never by grepping the production ledger. [SPRINT-40 BUG-069 post-flight]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F04: `pending-task-sentinel.sh:173` defaults agent_type to the LI
<!-- hash:71f84b -->

**Category:** stale
**Reason:** stale: zero grep hits across last 2 sprint dir(s)
**Original entry:** `2026-09-02 · #hooks #ledger #danger · `pending-task-sentinel.sh:173` defaults agent_type to the LITERAL string "unknown", not empty — so a refusal predicate testing only `[[ -z ]]` passes it straight through and a genuinely unattributable row still ships as "unknown". A `-z`-only impl scores identically to a correct one. Test the sentinel's DEFAULT VALUE, not just emptiness. [SPRINT-40 BUG-069 prep]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F05: `token-ledger.sh` claims the dispatch marker atomically at :
<!-- hash:248f2c -->

**Category:** stale
**Reason:** stale: zero grep hits across last 2 sprint dir(s)
**Original entry:** `2026-09-02 · #hooks #ledger #danger · `token-ledger.sh` claims the dispatch marker atomically at :213 BEFORE reading it, but claims the sentinel only at :471 — AFTER the `USAGE_JSON`-empty exit at :299 (reachable: a truncated final JSONL line makes `jq -cs` emit nothing). That path DESTROYS the marker and LEAKS the sentinel, so the next fire inherits the previous dispatch's attribution. No sweeper reaps orphan `.processed-*`. [SPRINT-40 BUG-068 post-flight]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F06: A rename-proof tool-name predicate INSIDE a hook is dead cod
<!-- hash:b71ccf -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-09-02 · #hooks #settings #danger · A rename-proof tool-name predicate INSIDE a hook is dead code when `settings.json`'s matcher is the outer routing gate: `"Task|Agent"` means the next host rename never reaches the hook and its rejection log line never fires. Sc3 only passes because the test pipes the payload direct, bypassing settings.json. Rename-proofing belongs in the matcher, or it is theatre. [SPRINT-40 BUG-068 post-flight]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F07: `pre-tool-use-task.sh:67` ALLOW_LIST names 5 agent roles (`a
<!-- hash:dada27 -->

**Category:** stale
**Reason:** stale: zero grep hits across last 2 sprint dir(s)
**Original entry:** `2026-09-01 · #hooks #ledger #danger · `pre-tool-use-task.sh:67` ALLOW_LIST names 5 agent roles (`architect developer qa reporter cleargate-wiki-contradict`) but `cleargate-planning/.claude/agents/` holds 11 — `devops`, `architect-reader`, `architect-synth` and the three other wiki roles write NO dispatch marker even with the tool-name guard fixed. DevOps fires on every story merge, so a "restored" ledger still has a permanent hole. Count the agents dir against the allow-list, not the allow-list against itself. [SPRINT-40 M1 BUG-068]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F08: A red test for `pending-task-sentinel.sh`'s flashcard barrie
<!-- hash:209528 -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-09-01 · #hooks #test-harness #danger · A red test for `pending-task-sentinel.sh`'s flashcard barrier is silently defeated by the ORCHESTRATOR's own env: `SKIP_FLASHCARD_GATE=1` skips the block (:53) and `CLEARGATE_ADVISORY=1` downgrades the exit-1 to a warning (:129) — both read as "test still red" with no clue why. `launch_wave.mjs:287` exports the first during every wave. Run barrier tests as `env -u SKIP_FLASHCARD_GATE -u CLEARGATE_ADVISORY`. [SPRINT-40 M1 BUG-068]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F09: The red-test immutability gate is TS-only: `pre-commit-surfa
<!-- hash:75a492 -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-09-01 · #hooks #immutability #danger · The red-test immutability gate is TS-only: `pre-commit-surface-gate.sh:11` matches `\.red\.(node\.)?test\.ts$`, so all 7 `*.red.sh` bash red tests in `.cleargate/scripts/test/` — including this sprint's sole spec artifact — can be edited by a Developer commit with zero hook objection. In a sprint whose surfaces are bash, red-test immutability is honour-system. Second instance of the same regex-vs-convention drift as the `.red.integration.node.test.ts` gap. [SPRINT-40 M1]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F10: A sprint plan's shared-file collision claim is a hypothesis,
<!-- hash:c81e01 -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-09-01 · #planning #dogfood-split · A sprint plan's shared-file collision claim is a hypothesis, not a measurement: SPRINT-40 §2.3 said both M1 items edit `token-ledger.sh`, but running the committed red test at baseline showed its marker-read path (:204-232) already intact — BUG-068 needs ZERO lines there. Run the red test before partitioning a shared surface; the failure pattern tells you which file actually has to change. [SPRINT-40 M1]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F11: Do NOT run `cleargate stamp` on `.cleargate/templates/*.md`.
<!-- hash:dfa287 -->
...
### CAND-SPRINT-40-F12: `cleargate-enforcement.md` sources its §1-§12 from "protocol
<!-- hash:bf2bac -->
...
### CAND-SPRINT-40-F13: "Verified BOTH redirected and unredirected" is vacuous unles
<!-- hash:a8c9ad -->
...
### CAND-SPRINT-40-F14: `pre-commit-surface-gate.sh:11` matches red tests with `\.re
<!-- hash:514de6 -->
...
### CAND-SPRINT-40-F15: Never write a BARE COUNT in a dispatch — enumerate and let t
<!-- hash:b3bd59 -->
...
### CAND-SPRINT-40-F16: `.cleargate/config.yml` `gates.precommit` is NOT wired to an
<!-- hash:51d1d3 -->
...
### CAND-SPRINT-40-F17: A test's HEADER COMMENT claiming it follows the `CLI_ROOT/ME
<!-- hash:01a1e9 -->
...
### CAND-SPRINT-40-F18: An assertion of the form "the real artefact FAILS this crite
<!-- hash:7af1b4 -->
...
### CAND-SPRINT-40-F19: A red test that pins a PARSE-ERROR throw (`assert.throws(/un
<!-- hash:1c9012 -->
...
### CAND-SPRINT-40-F20: For a grandfathering predicate the TRIGGER is the entire des
<!-- hash:01a648 -->
...
### CAND-SPRINT-40-F21: A regex/substring assertion over a fenced ```yaml block pass
<!-- hash:c9e2a2 -->
...
### CAND-SPRINT-40-F22: `npm test`'s stdout is BLOCK-buffered when redirected to a f
<!-- hash:a4d52e -->
...
### CAND-SPRINT-40-F23: A predicate-shape count lives in TWO places that drift indep
<!-- hash:7f4a86 -->
...
### CAND-SPRINT-40-F24: A frozen-value pin survived two TPV mutation rounds because 
<!-- hash:b24014 -->
...
### CAND-SPRINT-40-F25: A canonical-only edit RE-CREATES live/canonical line drift t
<!-- hash:f8cc9f -->
...
### CAND-SPRINT-40-F26: process.exit() does NOT unwind a pending try/finally — a loc
<!-- hash:ddd2b1 -->
...
### CAND-SPRINT-40-F27: A shape assertion pinned via a few NAMED-type fixtures leave
<!-- hash:cbdc65 -->
...
### CAND-SPRINT-40-F28: A subprocess spawned with cwd=<tmp fixture> cannot resolve -
<!-- hash:e00107 -->
...
### CAND-SPRINT-40-F29: A grep-over-prose acceptance half is spelling coverage, and 
<!-- hash:e4d3e7 -->
...
### CAND-SPRINT-40-F30: A template's own GUIDANCE prose satisfies a whole-file grep 
<!-- hash:de2727 -->
...
### CAND-SPRINT-40-F31: A synthetic future-state fixture silently DECIDES the implem
<!-- hash:584fdd -->
...
### CAND-SPRINT-40-F32: Cross-Cutting Rule 1 had exactly ONE machine witness in CR-1
<!-- hash:017d17 -->
...
### CAND-SPRINT-40-F33: stampFrontmatter's hasFrontmatter check (stamp-frontmatter.t
<!-- hash:ca7a56 -->
...
### CAND-SPRINT-40-F34: story.md's own token vocabulary contradicts the corpus it de
<!-- hash:fcd8a8 -->
...
### CAND-SPRINT-40-F35: For a module that does not exist yet, a per-test dynamic imp
<!-- hash:c48bba -->
...
### CAND-SPRINT-40-F36: A vocabulary-duplication guard must be enum- or co-occurrenc
<!-- hash:85c962 -->
...
### CAND-SPRINT-40-F37: cr078_init.test.sh's trailing SAFETY check hardcodes "expect
<!-- hash:bbf9aa -->
...
### CAND-SPRINT-40-F38: init_sprint.mjs resolves its template from CLEARGATE_REPO_RO
<!-- hash:da8f0c -->
...
### CAND-SPRINT-40-F39: A cross-process barrier that arms INSIDE the critical sectio
<!-- hash:bd4502 -->
...
### CAND-SPRINT-40-F40: A scenario's CLAIMED mutant is a HYPOTHESIS until measured. 
<!-- hash:8350b7 -->
...
### CAND-SPRINT-40-F41: git tracks no empty directories, so "directory absent" is th
<!-- hash:ba2a69 -->
...
### CAND-SPRINT-40-F42: A two-source fixture where the BUGGY source holds the higher
<!-- hash:048685 -->
...
### CAND-SPRINT-40-F43: check:no-inline-id-regex requires the escape to follow the h
<!-- hash:7f3624 -->
...
### CAND-SPRINT-40-F44: A grep-shaped doc assertion bans a PHRASING, not a CLAIM. BU
<!-- hash:c7e6b6 -->
...
### CAND-SPRINT-40-F45: Arming a REFUSAL changes the blast radius of every deferred 
<!-- hash:b09b0e -->
...
### CAND-SPRINT-40-F46: "Every work-item type" is not one set: work-item-id.ts:56 ex
<!-- hash:d148c7 -->
...
### CAND-SPRINT-40-F47: The template map is NOT ${type}.md — cr→CR.md, bug→Bug.md, s
<!-- hash:b956bb -->
...
### CAND-SPRINT-40-F48: cleargate-cli's 30-file integration tier is excluded by run-
<!-- hash:fb1978 -->
...
### CAND-SPRINT-40-F49: An "exactly one block" IMPLEMENTATION that counts START SUBS
<!-- hash:46fcfb -->
...
### CAND-SPRINT-40-F50: A red-set census predicated on "which tests import the modul
<!-- hash:d76ff8 -->
...
### CAND-SPRINT-40-F51: A doc-truth baseline that pins the block's POSITION and the 
<!-- hash:161955 -->
...
### CAND-SPRINT-40-F52: A declared surface that is RIGHT FOR THE WRONG REASON beats 
<!-- hash:5d9fb6 -->
...
### CAND-SPRINT-40-F53: A fixture that EXERCISES a behaviour is not coverage of it: 
<!-- hash:e6fa00 -->
...
### CAND-SPRINT-40-F54: A line citation repaired in the same commit that edits the c
<!-- hash:358eab -->
...
### CAND-SPRINT-40-F55: A milestone can serve NO clause of the sprint goal. "met | p
<!-- hash:03f7db -->
...
### CAND-SPRINT-40-F56: cleargate-cli/dist/cli.js is untracked and hand-built, so af
<!-- hash:843403 -->
...
### CAND-SPRINT-40-F57: A cleargate-cli defect fix that ships no CHANGELOG entry has
<!-- hash:f9aa8c -->
...
### CAND-SPRINT-40-F58: A shared-corpus equivalence test that probes a module's OUTP
<!-- hash:f197cb -->
...
### CAND-SPRINT-40-F59: A fix that NORMALISES CRLF→LF on the write path passed all 2
<!-- hash:71590b -->
...
### CAND-SPRINT-40-F60: NEVER blanket-copy canonical .claude/hooks over live: 3 of t
<!-- hash:e8299c -->
...
### CAND-SPRINT-40-F61: file_surface_diff.sh globs STORY-<num>_*.md ONLY, so a BUG-/
<!-- hash:fd3a4e -->
...
### CAND-SPRINT-40-F62: A §Reach audit that greps OUTWARD never reads the file it ju
<!-- hash:8eb353 -->
...
### CAND-SPRINT-40-F63: A section(N) criterion can be RIGHT for the template and WRO
<!-- hash:ea880e -->
...
### CAND-SPRINT-40-F64: The global `cleargate` binary is a REAL npm install (/opt/ho
<!-- hash:7cbd89 -->
...
### CAND-SPRINT-40-F65: The installed outer pre-commit hook (pre-commit-surface-gate
<!-- hash:2fb23f -->
...
### CAND-SPRINT-40-F66: CLAUDE.md:162 teaches `{TYPE}-{ID}-{Name}.md` but deriveBuck
<!-- hash:5d7800 -->
...
### CAND-SPRINT-40-F67: CLAUDE.md's canonical and live copies are NOT mirrors and ca
<!-- hash:c21614 -->
...
### CAND-SPRINT-40-F68: A red-window prediction must be MEASURED, not reasoned: STOR
<!-- hash:d53767 -->
...
### CAND-SPRINT-40-F69: gate.ts writes cached_gate_result (:305) BEFORE the severity
<!-- hash:75138a -->
...
### CAND-SPRINT-40-F70: Before bumping a hardcoded count, check for HOMONYMS in the 
<!-- hash:b328ae -->
...
### CAND-SPRINT-40-F71: work-item-type.ts PREFIX_MAP matches by `includes`, so NO or
<!-- hash:ed4615 -->
...
### CAND-SPRINT-40-F72: An agent executing IN a worktree writes its report to the WO
<!-- hash:5b38f4 -->
...
### CAND-SPRINT-40-F73: A test guarded `skip: !existsSync(path)` reports SKIPPED — n
<!-- hash:19e143 -->
...
### CAND-SPRINT-40-F74: A cleargate-cli test that reads the OUTER repo's working tre
<!-- hash:38c962 -->
...
### CAND-SPRINT-40-F75: Scope CLEARGATE_META_ROOT to TARGETED runs only. Setting it 
<!-- hash:bd618b -->
...
### CAND-SPRINT-40-F76: MEASURED registry-wide: 9 of 12 pinnable section(N) criteria
<!-- hash:9d8d6a -->
...
### CAND-SPRINT-40-F77: "Inserting a `## ` heading renumbers section(N)" covers one 
<!-- hash:52bb37 -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-08-27 · #gate #readiness-gates · "Inserting a `## ` heading renumbers section(N)" covers one direction only. DELETING a heading above a gated band slides the sections BELOW it upward — spike.md loses one heading and bullet-rich `## Prior work` / the Ambiguity Gate land in gated slots, turning 2 criteria green on an empty doc. Fixture pinning catches it only once a row exists; between merge and the gating story there is zero coverage. [SPRINT-39 STORY-054-01]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F78: A story amendment made in the MAIN checkout's working tree i
<!-- hash:6ead5c -->
...
### CAND-SPRINT-40-F79: `initiative.md` frontmatter ships `created_at_version: "clea
<!-- hash:9cfb13 -->
...
### CAND-SPRINT-40-F80: `npm --prefix cleargate-cli test -- <file>` does NOT filter 
<!-- hash:e2ec73 -->
...
### CAND-SPRINT-40-F81: `.cleargate/config.yml` (36 lines) and `cleargate-planning/.
<!-- hash:dff0e4 -->
...
### CAND-SPRINT-40-F82: A story's §3.1 file surface is an ESTIMATE written at decomp
<!-- hash:22156c -->
...
### CAND-SPRINT-40-F83: file_surface_diff.sh picks the active story by `state in ('I
<!-- hash:332b8f -->
...
### CAND-SPRINT-40-F84: cleargate-cli/.git/hooks holds only .sample files — ZERO ins
<!-- hash:e9d790 -->
...
### CAND-SPRINT-40-F85: A two-repo story has a merge ORDER only if one intermediate 
<!-- hash:99a9fb -->
...
### CAND-SPRINT-40-F86: backfill_hierarchy.mjs parseFm bails on `!raw.startsWith('--
<!-- hash:991152 -->
...
### CAND-SPRINT-40-F87: An enumerating test's hardcoded totals (`criteria.length ===
<!-- hash:388045 -->
...
### CAND-SPRINT-40-F88: A pin keyed by heading TEXT needs NO fixture edit when an in
<!-- hash:04d77e -->
...
### CAND-SPRINT-40-F89: A red test that fails its fixture-guard FIRST leaves every h
<!-- hash:85df1a -->
...
### CAND-SPRINT-40-F90: `cleargate hotfix new` writes the template VERBATIM (only {I
<!-- hash:fd5261 -->
...
### CAND-SPRINT-40-F91: A pinning test that enumerates `section(N)` CRITERIA out of 
<!-- hash:2de5f1 -->
...
### CAND-SPRINT-40-F92: backfill_hierarchy.mjs fills a missing sprint_cleargate_id f
<!-- hash:0ff71a -->
...
### CAND-SPRINT-40-F93: `gate check <abs-path>` roots the project on the FIXTURE's o
<!-- hash:feddaa -->
...
### CAND-SPRINT-40-F94: A worktree materializes TRACKED files only — gitignored root
<!-- hash:6fd4bc -->
...
### CAND-SPRINT-40-F95: Template `<instructions>` blocks are STRIPPED from authored 
<!-- hash:effade -->
...
### CAND-SPRINT-40-F96: The sprint ledger is written by token-ledger.sh (SubagentSto
<!-- hash:152a51 -->
...
### CAND-SPRINT-40-F97: A gate built from the same mental model as the defect inheri
<!-- hash:88fea3 -->
...
### CAND-SPRINT-40-F98: `dirname(import.meta.url)/../..` is one level too deep in a 
<!-- hash:e7066c -->
...
### CAND-SPRINT-40-F99: A parser duplicated into a payload .mjs (can't import the co
<!-- hash:0f6e97 -->
...
### CAND-SPRINT-40-F100: THREE ID regexes disagree on date-form ids: `BUG-2026-08-24`
<!-- hash:75953d -->
...
### CAND-SPRINT-40-F101: An upgrade run in a dirty tree can leave .install-manifest.j
<!-- hash:a574c8 -->
...
### CAND-SPRINT-40-F102: `npm i -g .` from a checkout carrying an uncommitted version
<!-- hash:a97688 -->
...
### CAND-SPRINT-40-F103: A sample that never exercises the code path disproves NOTHIN
<!-- hash:ac96fc -->
...
### CAND-SPRINT-40-F104: A criterion matching a magic token is dead unless the TEMPLA
<!-- hash:76e3a4 -->
...
### CAND-SPRINT-40-F105: The payload copier walks the WORKING TREE, so gitignored/unt
<!-- hash:6603d3 -->
...
### CAND-SPRINT-40-F106: "Every source degrades to an empty panel, never a throw" is 
<!-- hash:184e95 -->
...
### CAND-SPRINT-40-F107: A routing change that starts a REAL server turns a test fail
<!-- hash:81b01f -->
...
### CAND-SPRINT-40-F108: `String(obj)` is ALWAYS '[object Object]' — non-empty and no
<!-- hash:05dd84 -->
...
### CAND-SPRINT-40-F109: A failure message that names only the FALLBACK remedy reads 
<!-- hash:cae8b2 -->
...
### CAND-SPRINT-40-F110: An or_group whose alternatives read the SAME ref emit the sa
<!-- hash:fe13c6 -->
...
### CAND-SPRINT-40-F111: `statusLine.includes('🟢')` has no notion of NEGATION — a do
<!-- hash:ebd8af -->
...
### CAND-SPRINT-40-F112: When a parser is wrong and the document is right, the fix is
<!-- hash:d3a060 -->
...
### CAND-SPRINT-40-F113: A hook that always exits 0 and sends its child's output only
<!-- hash:f6e7ac -->
...
### CAND-SPRINT-40-F114: A hook test that reads the GITIGNORED live `.claude/` copy v
<!-- hash:a4bb3a -->
...
### CAND-SPRINT-40-F115: process.cwd() is NOT the project root — Claude Code's Bash t
<!-- hash:3fe207 -->
...
### CAND-SPRINT-40-F116: A bare `.cleargate/` dir does NOT prove a project: the hooks
<!-- hash:968272 -->
...
### CAND-SPRINT-40-F117: A test that asserts "X is absent here" by inheriting the amb
<!-- hash:77599a -->
...
### CAND-SPRINT-40-F118: A feature can be fully built, tested (53 green) and still un
<!-- hash:8a3098 -->
...
### CAND-SPRINT-40-F119: "The repo root is not a package" is a monorepo assumption — 
<!-- hash:3b5f9a -->
...
### CAND-SPRINT-40-F120: A switch on overwrite_policy missing a case routes that poli
<!-- hash:00c06c -->
...
### CAND-SPRINT-40-F121: An upgrade that iterates only the NEW manifest can never del
<!-- hash:ec7ed3 -->
...
### CAND-SPRINT-40-F122: main/module/exports["."] forced a 10.5MB CJS twin nobody imp
<!-- hash:2f30c1 -->
...
### CAND-SPRINT-40-F123: check:no-vitest is itself a NO-OP in all 3 packages: package
<!-- hash:6538cd -->
...
### CAND-SPRINT-40-F124: A shipped pre-commit hook must guard every `npm --prefix <di
<!-- hash:58f804 -->
...
### CAND-SPRINT-40-F125: mcp/cleargate-cli/admin are gitignored in the meta-repo, so 
<!-- hash:fedafa -->
...
### CAND-SPRINT-40-F126: Reading a file by its bare CLAUDE.md-documented path inside 
<!-- hash:05016b -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-07-27 · #dogfood #worktree · Reading a file by its bare CLAUDE.md-documented path inside a worktree dispatch lands the edit on the MAIN checkout — always prefix with the worktree root, and cross-check `git status` before committing. [SPRINT-38 CR-087]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F127: A gate can be "restored" at the script level and still be de
<!-- hash:28c933 -->
...
### CAND-SPRINT-40-F128: Gitignored runtime sentinels (.active) never exist in a link
<!-- hash:49e2f2 -->
...
### CAND-SPRINT-40-F129: pre-commit-surface-gate.sh runs `npm run check:no-vitest --p
<!-- hash:396f69 -->
...
### CAND-SPRINT-40-F130: An architect plan's per-leg red/green ledger can mis-predict
<!-- hash:233cc7 -->
...
### CAND-SPRINT-40-F131: Orchestrator rulings issued in DISPATCH text must be written
<!-- hash:30a96e -->
...
### CAND-SPRINT-40-F132: Red tests written from dispatch-only scope additions read as
<!-- hash:409358 -->
...
### CAND-SPRINT-40-F133: When a ruling changes a POLICY, fix EVERY place the old poli
<!-- hash:8b4d0b -->
...
### CAND-SPRINT-40-F134: A close_sprint test case that reaches exit 0 runs Steps 7/7.
<!-- hash:14da3c -->
...
### CAND-SPRINT-40-F135: close-sprint-reconcile.integration spawns the REAL outer clo
<!-- hash:e36024 -->
...
### CAND-SPRINT-40-F136: developer/qa agents write a report file only on the BLOCKED 
<!-- hash:ce8cfa -->
...
### CAND-SPRINT-40-F137: Wrapping full-suite `npm test` through run_script.sh collide
<!-- hash:03c375 -->
...
### CAND-SPRINT-40-F138: test_assert_work_item_files.node.test.ts:194 ('v1 warns-only
<!-- hash:701ab5 -->
...
### CAND-SPRINT-40-F139: Gate try/catch must wrap ONLY the fallible call, not exitFn(
<!-- hash:23afb9 -->
...
### CAND-SPRINT-40-F140: Per-story scaffold-retirement tests assert canonical+live-ou
<!-- hash:afb4b2 -->

**Category:** resolved
**Reason:** keyword found in a prior §6 Tooling section
**Original entry:** `2026-07-18 · #dogfood #test-harness · Per-story scaffold-retirement tests assert canonical+live-outer deletions (the surfaces a story owns); payload-mirror cleanliness is transitively covered by the once-per-wave prebuild — don't red a story on the derived payload. [SPRINT-38 051-02]`
**Suggested action:** approve to remove via `cleargate flashcard prune` (run /improve)

---

### CAND-SPRINT-40-F141: A cli node:test whose REPO_ROOT default resolves to the oute
<!-- hash:1889f3 -->
...
### CAND-SPRINT-40-F142: prebuild's copy-planning-payload.mjs HARDCODES src=<outer-ma
<!-- hash:35018a -->
...
### CAND-SPRINT-40-F143: init_sprint.mjs no longer writes an execution_mode key into 
<!-- hash:f596b8 -->
...
### CAND-SPRINT-40-F144: When a story deletes a module + rewrites PRIOR-sprint red te
<!-- hash:218bc8 -->
...
### CAND-SPRINT-40-F145: mcp/src/db/migrations/ has TWO 0009_*.sql files (0009_aspiri
<!-- hash:886916 -->
...
### CAND-SPRINT-40-F146: broker/package.json ships ONLY `ws` — no redis/http client. 
<!-- hash:e77903 -->
...
### CAND-SPRINT-40-F147: run_script.sh FORWARDS CLEARGATE_STATE_FILE/CLAUDE_PROJECT_D
<!-- hash:89c281 -->
...
### CAND-SPRINT-40-F148: Two concurrent same-tick app→connector WS sends can coalesce
<!-- hash:0fe173 -->
...
### CAND-SPRINT-40-F149: EOF is the SOLE turn terminus: a `result` → turn_result and 
<!-- hash:a800cd -->
...
### CAND-SPRINT-40-F150: Two disjoint error classes: in-band `is_error:true` → turn_r
<!-- hash:5c7ec4 -->
...
### CAND-SPRINT-40-F151: claude tool names arrive already-PascalCase from the assista
<!-- hash:8d5358 -->
...
### CAND-SPRINT-40-F152: exactOptionalPropertyTypes=true: never assign `prop: expr|un
<!-- hash:9b5f08 -->
...
### CAND-SPRINT-40-F153: Node 25: `--input-type=module` is stdin/`--eval`/`--print` o
<!-- hash:a25503 -->
...
### CAND-SPRINT-40-F154: Developer makes the FROZEN *.red.node.test.ts pass — do NOT 
<!-- hash:73170e -->
...
### CAND-SPRINT-40-F155: Error subclass carrying `cause`: pass via `super(msg, { caus
<!-- hash:592d44 -->
...
### CAND-SPRINT-40-F156: A red test that greps for "zero hits" of a token will SELF-H
<!-- hash:bea9ae -->
...
### CAND-SPRINT-40-F157: cleargate init reads ROOT templates/cleargate-planning (reso
<!-- hash:5e8752 -->
...
### CAND-SPRINT-40-F158: TWO files shell 'npm pack --workspace=cleargate-cli': test/c
<!-- hash:76dcd1 -->
...
### CAND-SPRINT-40-F159: `node --test --import tsx/esm` throws ERR_REQUIRE_CYCLE_MODU
<!-- hash:c505bd -->
...
### CAND-SPRINT-40-F160: A Class-3 close_sprint.mjs gate (e.g. deferred_verification,
<!-- hash:51c0f0 -->
...
### CAND-SPRINT-40-F161: A Class-3 pre-gate check wired into pre_gate_runner.sh (e.g.
<!-- hash:d033ac -->
...
### CAND-SPRINT-40-F162: `npm run <x> --workspace=<pkg>` fails "No workspaces found" 
<!-- hash:6223d1 -->
...
### CAND-SPRINT-40-F163: `X=$(grep -c PAT f || echo 0)` DOUBLES to "0\n0" on zero mat
<!-- hash:0a46f6 -->
...
### CAND-SPRINT-40-F164: pre_gate_runner.sh arch-mode (~L205) runs `cd "$WORKTREE" &&
<!-- hash:038335 -->
...
### CAND-SPRINT-40-F165: write_dispatch.sh guard FIXED (043-09 re-verify): skip requi
<!-- hash:52d742 -->
...
### CAND-SPRINT-40-F166: A gap-closing red test can be GREEN on today's baseline by d
<!-- hash:417d77 -->
