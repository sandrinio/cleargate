...
- [`STORY-002-01`](../stories/STORY-002-01_Protocol_Section_10.md): Protocol §10 — add "Knowledge Wiki Protocol" to `cleargate-protocol.md` (ingest/query/lint, backlink syntax `[[ID]]`, gate-blocking rules, page schema, `log.md` YAML event format). **Path update:** edit `.cleargate/knowledge/cleargate-protocol.md` (not `strategy/knowledge/` — moved in Phase 2b). Mirror to `cleargate-planning/.cleargate/knowledge/cleargate-protocol.md`. · L1
- [`STORY-002-02`](../stories/STORY-002-02_Wiki_Ingest_Subagent.md): `cleargate-wiki-ingest` subagent definition (model: haiku). Triggered by PostToolUse hook on write to `.cleargate/delivery/**`, `.cleargate/plans/**`, or `strategy/work-items/**`. Writes per-item wiki page + log.md entry + recompiles affected synthesis pages. Ships as canonical source at `cleargate-planning/.claude/agents/cleargate-wiki-ingest.md` (installed via `cleargate init`). · L2
- [`STORY-002-03`](../stories/STORY-002-03_Wiki_Query_Subagent.md): `cleargate-wiki-query` subagent definition (model: haiku). Auto-invoked at triage (read-only mode) and on `cleargate wiki query --persist` (file-back mode → `wiki/topics/<slug>.md` with `cites:` frontmatter). · L1
- [`STORY-002-04`](../stories/STORY-002-04_Wiki_Lint_Subagent.md): `cleargate-wiki-lint` subagent definition (model: sonnet). Enforcement mode: refuses Gate 1 (Proposal approval) + Gate 3 (Push) on drift. `--suggest` advisory mode: surfaces candidate cross-refs ingest missed, exit 0. · L2
- [`STORY-002-05`](../stories/STORY-002-05_Init_Writes_Hook.md): `cleargate init` writes PostToolUse hook config into `.claude/settings.json` + copies the three subagent defs from `cleargate-cli/templates/.claude/agents/cleargate-wiki-*.md` → target repo's `.claude/agents/`. Adds a bootstrap step: after init completes, if raw items exist (`.cleargate/delivery/**` or `strategy/work-items/**`), auto-run `cleargate wiki build` once. · L2
- [`STORY-002-06`](../stories/STORY-002-06_Wiki_Build_CLI.md): `cleargate wiki build` — full rebuild from raw. Scans raw state, produces full `wiki/` tree (index.md + log.md + per-item pages + 4 synthesis pages + sprint pages). Idempotent (re-running produces byte-identical output modulo `last_ingest` timestamps). Uses git SHA (`git log -1 --format=%H <file>`) as the `codebase_version` substitute — no dependency on EPIC-001 stamp. · L2
- [`STORY-002-07`](../stories/STORY-002-07_Wiki_Ingest_CLI.md): `cleargate wiki ingest <file>` — single-file update (called by hook + subagent). Triggers recompile of the affected per-item page + every synthesis page that references it. · L2
- [`STORY-002-08`](../stories/STORY-002-08_Wiki_Lint_CLI.md): `cleargate wiki lint` + `cleargate wiki lint --suggest` + `cleargate wiki query <q> [--persist]`. Lint: exit non-zero on drift, names the offending page. Suggest: exit 0, prints candidate cross-refs. Query: reads wiki, synthesizes answer; with `--persist` files to `wiki/topics/<slug>.md`. · L2
- [`STORY-002-09`](../stories/STORY-002-09_Synthesis_Templates.md): four synthesis page templates (`active-sprint.md`, `open-gates.md`, `product-state.md`, `roadmap.md`) + compile recipes. Each recipe is a pure function `raw-state → markdown`, deterministic, rerun on every ingest. · L1
...
- **A1 — `repo:` frontmatter tag.** Every wiki page gets a `repo: cli | mcp | planning` tag derived from `raw_path` prefix (`cleargate-cli/` → cli, `mcp/` → mcp, `strategy/` or `cleargate-planning/` → planning). wiki-query filters by tag on user request. No new directory planes; tag-based discrimination only. Story touch: STORY-002-02 (ingest) + STORY-002-03 (query) + STORY-002-08 (lint — adds a check that `raw_path` and `repo:` agree).
...
- **A3 — Dual-source ingest during migration.** Raw sources include BOTH `.cleargate/delivery/pending-sync/` + `.cleargate/delivery/archive/` (post-migration) AND `strategy/work-items/{sprints,epics,stories}/` + `strategy/proposals/` (current transitional state). wiki-ingest and `wiki build` scan both; the file migration Phase 2c stays deferred without blocking wiki work. Story touch: STORY-002-06 (build scans both) + STORY-002-07 (ingest accepts either path).

**Total: 9 stories, all L1/L2. No L3. Adaptations A1-A3 fit within existing stories.**
...
- **Phase 2a + 2b shipped ✓** (committed today, 2026-04-19). `.cleargate/{knowledge,templates,delivery}/` scaffold exists at meta-root; `cleargate-planning/` canonical source exists; CLAUDE.md bounded block is in place.
- **Phase 2c NOT required**. Dual-source ingest (adaptation A3) supports the transitional state where `strategy/work-items/` hasn't migrated to `.cleargate/delivery/`.
...
- **Test infra:** Vitest (already configured in `cleargate-cli/`). No database needed — wiki is pure filesystem ops. Fixtures: create a temporary `.cleargate/delivery/` + `strategy/work-items/` tree under `os.tmpdir()`, invoke `wiki build`, assert output shape.
...
- **Multi-repo boundary:** meta-root wiki covers meta-root work items only (current strategy/work-items/ + future .cleargate/delivery/). mcp/ and cleargate-cli/ are NOT ingested by meta-root wiki — they're separate git scopes with their own (future) wikis. Architect plan calls this out explicitly to prevent scope bleed.
...
- Editing any file under `.cleargate/delivery/**` OR `strategy/work-items/**` triggers the PostToolUse hook; affected wiki pages refresh within 5s.
