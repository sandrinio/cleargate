...
§0.1 v2 Decomposition Signals:
  ...
  `lane`: "standard" | "fast" — Architect-set during Sprint Design Review per the seven-check rubric in protocol §9. Default "standard". Absent in pre-EPIC-022 stories means standard per the migration default in update_state.mjs.
...
> **v2 gate input:** under v2 execution mode, this table is a pre-commit gate input (protocol §6). Every file staged in this story's commit must appear in the Value column, or be covered by `.cleargate/scripts/surface-whitelist.txt`. Non-path rows (e.g. "Mirrors", "New Files Needed: Yes/No") are ignored by the parser.
